#!/bin/bash
# Read-only status for the one-shot exploratory smoke recovery.

set -euo pipefail

[[ $# -eq 0 ]] || {
  echo 'Usage: scripts/status_massive_medical_union_composition_exploratory_smoke_recovery_v1_tillicum.sh' >&2
  exit 2
}
...
if [[ -e $control/SMOKE_RECOVERY_CPU_PREFLIGHT.json ]]; then
...
if [[ -e $control/SMOKE_RECOVERY_JOB.json ]]; then
  job_id=$(python -c 'import json,sys; print(json.load(open(sys.argv[1]))["job_id"])' "$control/SMOKE_RECOVERY_JOB.json")
  ...
  if [[ -e $control/SMOKE_RECOVERY_RESULT.json ]]; then
    ...
    case $state in
      ...
      FAILED*|CANCELLED*|TIMEOUT*|OUT_OF_MEMORY*|NODE_FAIL*|PREEMPTED*)
        ...
        if [[ -e $control/STOPPED_smoke_recovery ]]; then
          cat "$control/STOPPED_smoke_recovery"
        else
  ...
  elif [[ -e $control/STOPPED_smoke_recovery ]]; then
