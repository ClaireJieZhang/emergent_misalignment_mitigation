#!/bin/bash
# CPU-stage, derive, and audit job 271409's batch-2 result in fresh recovery v2.
...
[[ $# -eq 0 ]] || {
  echo 'Usage: run_massive_medical_kalai_s1_batch2_result_recovery_v2_tillicum.sh' >&2
  exit 2
}
...
failed_repo=$root/projects/subliminal-mitigate-mmu-kalai-s1-batch2-result-recovery-v1
failed_output=$root/outputs/massive_medical_kalai_s1_batch2_result_recovery_v1
output=$root/outputs/massive_medical_kalai_s1_batch2_result_recovery_v2
env_root=$root/envs/subliminal-mitigate-py311

test "$(basename "$repo")" = subliminal-mitigate-mmu-kalai-s1-batch2-result-recovery-v2
...
test -d "$failed_repo/.git"
test "$(git -C "$failed_repo" rev-parse HEAD)" = 6460a9895919d72f97b870f802b9d0813a0c45e2
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-batch2-recovery-v2-pyc
export DO_NOT_TRACK=1 HF_HUB_DISABLE_TELEMETRY=1 HF_HUB_OFFLINE=1
export TRANSFORMERS_OFFLINE=1 HF_DATASETS_OFFLINE=1

cd "$repo"
python -m pip check
bash -n scripts/run_massive_medical_kalai_s1_batch2_result_recovery_v2_tillicum.sh
python -m py_compile \
...
python -m unittest tests.test_massive_medical_kalai_s1_batch2_result_recovery_v2
python scripts/manage_massive_medical_kalai_s1_batch2_result_recovery_v2.py --self-test

common_args=(
  --source-output-root "$source_output"
  --source-repo-root "$source_repo"
  --failed-recovery-repo-root "$failed_repo"
  --failed-recovery-output-root "$failed_output"
  --output-root "$output"
  --repo-root "$repo"
)
python scripts/manage_massive_medical_kalai_s1_batch2_result_recovery_v2.py \
  --stage "${common_args[@]}"
python scripts/manage_massive_medical_kalai_s1_batch2_result_recovery_v2.py \
  --stage "${common_args[@]}"
python scripts/manage_massive_medical_kalai_s1_batch2_result_recovery_v2.py \
...
test -z "$(git -C "$failed_repo" status --porcelain)"
test -z "$(git -C "$repo" status --porcelain)"
echo KALAI_S1_BATCH2_RESULT_RECOVERY_V2_CPU_COMPLETE_NO_GPU_OR_API
