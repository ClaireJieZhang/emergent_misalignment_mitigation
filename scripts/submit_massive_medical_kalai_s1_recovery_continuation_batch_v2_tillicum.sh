#!/bin/bash
# Separately authorize, held-audit, and release one continuation-v2 batch.
...
usage() {
  echo 'Usage: submit_massive_medical_kalai_s1_recovery_continuation_batch_v2_tillicum.sh --batch-index N --ack-h200-minutes 60 --ack-max-cost-usd 0.900 --ack-current-conservative-exposure-usd X --ack-conservative-program-max-usd Y --ack-program-ceiling-usd 12.5000000 --ack-no-api --ack-no-automatic-next-batch --ack-no-restart-resume-retry-replacement --ack-prior-caps-retained --ack-recovered-batch2-predecessor --ack-post-hoc-sensitivity' >&2
  exit 2
}
...
batch_index=$2; minutes=$4; cost=$6; current=$8; maximum=${10}; ceiling=${12}
shift 12
[[ $1 == --ack-no-api && $2 == --ack-no-automatic-next-batch && $3 == --ack-no-restart-resume-retry-replacement && $4 == --ack-prior-caps-retained && $5 == --ack-recovered-batch2-predecessor && $6 == --ack-post-hoc-sensitivity ]] || usage
[[ $batch_index =~ ^[3-7]$ ]] || usage
[[ $minutes == 60 && $cost == 0.900 && $ceiling == 12.5000000 ]] || usage
printf -v batch_id 'batch_%02d' "$batch_index"

root=/gpfs/projects/stf/claizhan/subliminal-mitigate
repo=$root/projects/subliminal-mitigate-mmu-kalai-s1-recovery-continuation-v2
output=$root/outputs/massive_medical_kalai_s1_recovery_continuation_v2
source_output=$root/outputs/massive_medical_kalai_s1_recovery_continuation_v1
recovery_output=$root/outputs/massive_medical_kalai_s1_batch2_result_recovery_v1
control=$output/control/batches/$batch_id
manager=$repo/scripts/manage_massive_medical_kalai_s1_recovery_continuation_v2.py
authorizer=$repo/scripts/authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py
sbatch_file=$repo/scripts/sbatch_massive_medical_kalai_s1_recovery_continuation_batch_v2_tillicum_h200.sbatch
expected_name=mmu_kalai_s1_r2c$(printf '%02d' "$batch_index")
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-rc-v2-$batch_id-submit-pyc
unset OPENAI_API_KEY

python "$manager" preflight --output-root "$output" --repo-root "$repo" --batch-index "$batch_index"
python "$authorizer" preflight --output-root "$output" --repo-root "$repo" --batch-index "$batch_index"
mkdir -p "$output/control/batches"
mkdir "$control" "$control/SUBMISSION_LOCK"
owner_tmp=$control/SUBMISSION_LOCK/owner.tmp.$$
printf 'protocol_id=massive_medical_kalai_s1_recovery_continuation_v2\nstage=recovery_continuation_batch\nbatch_id=%s\nrepository_commit=%s\nrestart_or_resume_authorized=false\nretry_or_replacement_authorized=false\nautomatic_next_batch_authorized=false\n' "$batch_id" "$(git rev-parse HEAD)" > "$owner_tmp"
chmod 0400 "$owner_tmp"; mv "$owner_tmp" "$control/SUBMISSION_LOCK/owner"

python "$authorizer" write --output-root "$output" --repo-root "$repo" \
  --batch-index "$batch_index" --ack-h200-minutes "$minutes" \
  --ack-max-cost-usd "$cost" --ack-current-conservative-exposure-usd "$current" \
  --ack-conservative-program-max-usd "$maximum" --ack-program-ceiling-usd "$ceiling" \
  --ack-no-api --ack-no-automatic-next-batch \
  --ack-no-restart-resume-retry-replacement --ack-prior-caps-retained \
  --ack-recovered-batch2-predecessor --ack-post-hoc-sensitivity
...
raw_job=$(sbatch --parsable --hold --export=NONE --no-requeue \
  --account=stf --partition=gpu-h200 --qos=normal --nodes=1 --ntasks=1 \
  --cpus-per-task=8 --mem=200G --gres=gpu:h200:1 --time=01:00:00 \
  --job-name="$expected_name" "$sbatch_file" "$batch_index")
job_id=${raw_job%%;*}
[[ $job_id =~ ^[0-9]+$ ]]
released=false
...
chmod 0400 "$attempt_tmp"; mv "$attempt_tmp" "$control/SUBMISSION_ATTEMPT.tsv"
...
chmod 0400 "$submitted_tmp"; mv "$submitted_tmp" "$control/SUBMITTED"
release_tmp=$control/RELEASE_AUTHORIZED.tmp.$$
printf 'protocol_id=massive_medical_kalai_s1_recovery_continuation_v2\nstage=recovery_continuation_batch\nbatch_id=%s\njob_id=%s\nheld_audit_passed=true\nrelease_authorized=true\nrestart_or_resume_authorized=false\nautomatic_next_batch_authorized=false\n' "$batch_id" "$job_id" > "$release_tmp"
chmod 0400 "$release_tmp"; mv "$release_tmp" "$control/RELEASE_AUTHORIZED"
scontrol release "$job_id"; released=true
released_tmp=$control/RELEASED.tmp.$$
printf 'protocol_id=massive_medical_kalai_s1_recovery_continuation_v2\nstage=recovery_continuation_batch\nbatch_id=%s\njob_id=%s\nreleased=true\nrestart_or_resume_authorized=false\nautomatic_next_batch_authorized=false\n' "$batch_id" "$job_id" > "$released_tmp"
chmod 0400 "$released_tmp"; mv "$released_tmp" "$control/RELEASED"
trap - EXIT
echo "Submitted and released one-shot continuation-v2 $batch_id as job $job_id."
