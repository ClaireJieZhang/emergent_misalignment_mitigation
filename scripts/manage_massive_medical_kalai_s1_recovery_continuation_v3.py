...
EXPECTED_REPO_LEAF = "subliminal-mitigate-mmu-kalai-s1-recovery-continuation-v3"
...
EXPECTED_RECOVERY_REPO_LEAF = "subliminal-mitigate-mmu-kalai-s1-batch2-result-recovery-v2"
EXPECTED_RECOVERY_OUTPUT_LEAF = "massive_medical_kalai_s1_batch2_result_recovery_v2"
...
_NEW_FILES = (
    "configs/pipelines/massive_medical_kalai_s1_recovery_continuation_v3.yaml",
    "docs/massive_medical_kalai_s1_recovery_continuation_v3_protocol.md",
    ...
    "tests/test_massive_medical_kalai_s1_recovery_continuation_v3.py",
)
...
def _verify_recovered_result(paths, recovery_repo):
    ...
    if (
        ...
        or body.get("failed_recovery_v1_output_absent") is not True
        or body.get("failed_recovery_v1_rerun") is not False
...
def _expected_plan_body(context, output_root):
    ...
    return {
        ...
        "failed_recovery_v1": context["recovery_body"]["failed_recovery_v1"],
        "recovered_batch_2": {
            "batch_index": 2,
            "batch_id": "batch_02",
        ...
        "preserved_terminal_history": {
            "batch_1_stopped": True,
            "batch_2_stopped": True,
            "failed_recovery_v1_repository": True,
...
def _audit_namespace(output_root, allowed_control):
    ...
    if output.name != EXPECTED_OUTPUT_LEAF:
        raise ValueError("recovery-continuation-v3 output leaf differs")
    if os.path.lexists(output) and (output.is_symlink() or not output.is_dir()):
        raise ValueError("recovery-continuation-v3 output is unsafe")
...
def prepare(args):
    ...
    disposition = _write_idempotent(raw_output.resolve() / "control" / PLAN_NAME, plan, "recovery-continuation-v3 plan")
    print(json.dumps({"status": f"KALAI_S1_RECOVERY_CONTINUATION_V3_PLAN_{disposition}", "plan_payload_sha256": plan[SEAL_FIELD], "continuation_batches": list(BATCH_INDICES), "gpu_jobs": 0, "external_api_calls": 0}, sort_keys=True))
    return plan, context


def load_and_verify_plan(path, *, audit_source=True):
    path = Path(path).resolve()
    payload = load_json(path, "recovery-continuation-v3 plan")
    body = verify_seal(payload, "recovery-continuation-v3 plan")
    output = Path(body.get("output_root", "")).resolve()
    if path != output / "control" / PLAN_NAME:
        raise ValueError("recovery-continuation-v3 plan path differs")
    context = None
    if audit_source:
        ...
        if body != _expected_plan_body(context, output):
            raise ValueError("recovery-continuation-v3 plan differs from sources")
        ...
        raise ValueError("recovery-continuation-v3 plan identity differs")
...
def stage(args):
    ...
    if repo.name != EXPECTED_REPO_LEAF:
        raise ValueError("recovery-continuation-v3 repository leaf differs")
    ...
    require_clean(repo, "recovery-continuation-v3")
    plan, _ = prepare(args)
    ...
    disposition = _write_idempotent(output / "control" / STAGE_NAME, stage_payload, "recovery-continuation-v3 CPU stage")
    load_and_verify_stage(output / "control" / STAGE_NAME, repo, audit_source=True)
    print(json.dumps({"status": f"KALAI_S1_RECOVERY_CONTINUATION_V3_CPU_STAGE_{disposition}", "plan_payload_sha256": plan[SEAL_FIELD], "cpu_stage_payload_sha256": stage_payload[SEAL_FIELD], "batch_authorizations_created": 0, "gpu_jobs": 0, "external_api_calls": 0}, sort_keys=True))
...
def load_and_verify_stage(path, repo_root, *, audit_source=True):
    path = Path(path).resolve()
    repo = Path(repo_root).resolve()
    payload = load_json(path, "recovery-continuation-v3 CPU stage")
    body = verify_seal(payload, "recovery-continuation-v3 CPU stage")
    ...
    if body.get("protocol_id") != PROTOCOL_ID or body.get("status") != "CPU_STAGED_NO_GPU_OR_API_AUTHORITY" or body.get("new_cost_cap_usd") != 0.0 or body.get("gpu_jobs") != 0 or body.get("external_api_calls") != 0 or policy != expected_policy or set(repository) != {"path", "commit", "clean"} or repository.get("path") != str(repo) or repository.get("clean") is not True:
        raise ValueError("recovery-continuation-v3 CPU-stage policy differs")
    require_clean(repo, "recovery-continuation-v3")
    if git_commit(repo) != repository.get("commit") or body.get("implementation_sha256") != implementation_bindings(repo):
        raise ValueError("recovery-continuation-v3 implementation binding differs")
    ...
    if plan_binding != binding(plan_binding["path"], plan) or path != Path(plan_body["output_root"]) / "control" / STAGE_NAME:
        raise ValueError("recovery-continuation-v3 stage path differs")
...
def preflight(args):
    ...
    print(json.dumps({"status": "KALAI_S1_RECOVERY_CONTINUATION_V3_BATCH_PREFLIGHT_VALID", "batch_id": batch_id(args.batch_index), "continuation_plan_payload_sha256": plan[SEAL_FIELD], "recovered_batch_2_bound": True, "reference_models_loaded": 0, "gpu_jobs": 0, "external_api_calls": 0}, sort_keys=True))
...
def self_test():
    ...
    print("MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_V3_SELF_TEST_OK")
