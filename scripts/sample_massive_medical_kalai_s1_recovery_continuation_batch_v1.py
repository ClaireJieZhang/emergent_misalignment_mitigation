#!/usr/bin/env python3
"""Run one fresh recovery-continuation batch using the sealed source plan."""
...
manager = _load(
    "_kalai_s1_recovery_continuation_manager_for_runtime",
    "manage_massive_medical_kalai_s1_recovery_continuation_v1.py",
)
authorizer = _load(
    "_kalai_s1_recovery_continuation_authorizer_for_runtime",
...
def _call_with_continuation_protocol(function, *args, **kwargs):
    observed = manager.original_runtime.PROTOCOL_ID
    try:
        manager.original_runtime.PROTOCOL_ID = manager.PROTOCOL_ID
        return function(*args, **kwargs)
    finally:
        manager.original_runtime.PROTOCOL_ID = observed
...
def audit(args):
    ...
    result = _call_with_continuation_protocol(
        ...
        json.dumps(
            {
                ...
                "combined_timing_payload_sha256": result[
...
def generate(args):
    ...
    if Path(args.gpu_authorization).resolve() != authorizer.authorization_path(
        output, args.batch_index
    ):
        raise ValueError("GPU authorization path differs")
    return _call_with_continuation_protocol(
        manager.original_runtime._run_generation,
        argparse.Namespace(
            output_root=str(output),
            repo_root=args.repo_root,
            batch_index=args.batch_index,
            ...
            gpu_authorization=args.gpu_authorization,
        ),
        context["source_plan"],
        context["source_body"],
    )
...
def self_test():
    ...
    original = manager.original_runtime.PROTOCOL_ID
    observed = _call_with_continuation_protocol(
        lambda: manager.original_runtime.PROTOCOL_ID
    )
    assert observed == manager.PROTOCOL_ID
    assert manager.original_runtime.PROTOCOL_ID == original
    print("MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_RUNTIME_SELF_TEST_OK")
