#!/bin/bash
# CPU-only staging and finalization of the sealed smoke gate recovery v5.

set -euo pipefail
umask 077

[[ $# -eq 0 ]] || {
  echo 'Usage: scripts/stage_massive_medical_union_composition_exploratory_smoke_gate_recovery_v5_tillicum.sh' >&2
  exit 2
}
...
REMOTE_BRANCH=${REMOTE_BRANCH:-claire/capability-quorum-secure-code-composition-exploratory-smoke-gate-recovery-v5}
source_commit=f2c9c53cd63465f2a773eb13ed64f09cc5126b89
...
test "$(wc -l <<<"$observed_diff" | tr -d ' ')" -eq 5
...
branch=$3
expected_commit=$4
repo=$root/projects/subliminal-mitigate-mmu-composition-exploratory-smoke-gate-recovery-v5
output=$root/outputs/massive_medical_union_composition_exploratory_smoke_gate_recovery_v5
...
failed_v4_repo=$root/projects/subliminal-mitigate-mmu-composition-exploratory-smoke-gate-recovery-v4
failed_v4_output=$root/outputs/massive_medical_union_composition_exploratory_smoke_gate_recovery_v4
...
test -d "$failed_v4_repo"
test -f "$failed_v4_output/control/PREP.json"
test -f "$env_root/.ready"
if compgen -G "$root/outputs/logs/massive_medical_union_composition_exploratory_smoke_gate_recovery_v5_*" >/dev/null; then
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-composition-smoke-gate-recovery-v5-stage-pyc
...
auditor=scripts/audit_massive_medical_union_composition_exploratory_smoke_gate_recovery_v5.py
...
echo 'CPU-only exploratory smoke gate recovery v5 completed and sealed.'
echo 'No Slurm job, GPU allocation, model load, generation, API call, or confirmation was authorized.'
echo "Read-only status: ssh $TILLICUM_HOST '$TILLICUM_ROOT/projects/subliminal-mitigate-mmu-composition-exploratory-smoke-gate-recovery-v5/scripts/status_massive_medical_union_composition_exploratory_smoke_gate_recovery_v5_tillicum.sh'"
