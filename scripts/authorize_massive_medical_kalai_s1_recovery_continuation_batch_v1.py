#!/usr/bin/env python3
"""Authorize exactly one recovery-continuation batch in indices 2--7."""
...
def _plan_summary(plan_body, batch_index):
    ...
    if (
        ...
        or [item.get("batch_index") for item in batches] != list(BATCH_INDICES)
    ):
        raise ValueError("continuation batch inventory differs")
    return batches[batch_index - 2]["summary"]


def predecessor_binding(output_root, repo_root, plan_body, batch_index):
    if batch_index == 2:
        expected = plan_body.get("recovery_bindings", {}).get(
            "RECOVERED_RESULT.json"
        )
        payload = manager.load_json(
            expected.get("path", ""), "recovered batch-1 predecessor"
        )
        body = manager.recovery_planner.verify_seal(
            payload, "recovered batch-1 predecessor"
        )
        ...
        if (
            ...
            or body.get("batch_2_submission_authorized") is not False
        ):
            raise ValueError("recovered batch-1 predecessor differs")
        return {"kind": "recovered_batch_1_result", "artifact": observed}

    evaluator = _load(
        f"_kalai_s1_recovery_continuation_evaluator_for_predecessor_{batch_index}",
        "evaluate_massive_medical_kalai_s1_recovery_continuation_batch_v1.py",
    )
    previous = evaluator.load_and_verify_result(
        output_root, repo_root, batch_index - 1, audit_generation=True
    )
    stopped = control_root(output_root, batch_index - 1) / "STOPPED"
    if os.path.lexists(stopped):
        raise ValueError("preceding recovery-continuation batch is stopped")
    result_path = control_root(output_root, batch_index - 1) / "RESULT.json"
    return {
        "kind": "preceding_recovery_continuation_batch_result",
        "artifact": manager.binding(result_path, previous),
    }
...
def expected_body(output_root, repo_root, batch_index, created_at):
    ...
    return {
        ...
        "predecessor": predecessor_binding(
            output, repo_root, plan_body, batch_index
        ),
...
def audit_acknowledgments(args):
    ...
    if (
        ...
        or Decimal(args.ack_current_conservative_exposure_usd)
        != manager.current_exposure(args.batch_index)
        or Decimal(args.ack_conservative_program_max_usd)
        != manager.maximum_exposure(args.batch_index)
    ...
    for name in (
        ...
        "ack_prior_caps_retained",
        "ack_recovered_batch1_predecessor",
...
def audit_control_state(
    output_root, repo_root, batch_index, *, running_job_id=None
):
    control = control_root(output_root, batch_index)
    if control.is_symlink() or not control.is_dir():
        raise ValueError("target batch control is absent or unsafe")
    base = {"SUBMISSION_LOCK", "AUTHORIZATION.json"}
    ...
    if _lines(control / "SUBMISSION_LOCK" / "owner", "submission lock") != _expected_lock(
        repo_root, batch_index
    ):
        raise ValueError("submission-lock owner differs")
    if running_job_id is not None:
        batch_name = manager.batch_id(batch_index)
        if _lines(control / "SUBMISSION_ATTEMPT.tsv", "submission attempt") != {
            "stage\tbatch_id\tjob_id\th200_minutes\tmaximum_cost_usd",
            f"recovery_continuation_batch\t{batch_name}\t{running_job_id}\t60\t0.900",
        }:
        ...
        common = {
            ...
            f"job_id={running_job_id}",
            "restart_or_resume_authorized=false",
            "automatic_next_batch_authorized=false",
        }
        if _lines(control / "SUBMITTED", "submitted record") != common | {
            "held_first=true",
            "held_audit_passed=true",
            f"repository_commit={manager.git_commit(repo_root)}",
        }:
            raise ValueError("submitted record differs")
        if _lines(control / "RELEASE_AUTHORIZED", "release authority") != common | {
            "held_audit_passed=true",
            "release_authorized=true",
        }:
            raise ValueError("release authority differs")
        if _lines(control / "RELEASED", "release record") != common | {
            "released=true"
        }:
...
def preflight_predecessor(args):
    """Deeply validate the predecessor before creating any target state."""
    manager._require_no_api_key()
    output, plan, plan_body, _, _, _ = manager._load_workflow(
        args.output_root, args.repo_root, audit_source=True
    )
    _require_target_fresh(output, args.batch_index)
    control = control_root(output, args.batch_index)
    if os.path.lexists(control):
        raise ValueError("target batch control already exists")
    predecessor = predecessor_binding(
        output, args.repo_root, plan_body, args.batch_index
    )
    summary = _plan_summary(plan_body, args.batch_index)
    print(
        json.dumps(
            {
                "status": "KALAI_S1_RECOVERY_CONTINUATION_PREDECESSOR_PREFLIGHT_VALID",
...
def write_authorization(args):
    ...
    if (
        ...
        or {item.name for item in control.iterdir()} != {"SUBMISSION_LOCK"}
        or _lines(control / "SUBMISSION_LOCK" / "owner", "submission lock")
        != _expected_lock(args.repo_root, args.batch_index)
    ):
        raise ValueError("pre-authorization control state differs")
    created_at = dt.datetime.now(dt.timezone.utc).isoformat()
    payload = manager.seal(
        expected_body(
            args.output_root, args.repo_root, args.batch_index, created_at
        )
    )
    ...
    print(
        json.dumps(
            {
                "status": "KALAI_S1_RECOVERY_CONTINUATION_BATCH_AUTHORIZED",
                "batch_id": manager.batch_id(args.batch_index),
...
def verify_authorization(args):
    manager._require_no_api_key()
    payload = manager.load_json(
        authorization_path(args.output_root, args.batch_index),
        "batch authorization",
    )
    ...
    audit_control_state(
        ...
        args.repo_root,
        args.batch_index,
        running_job_id=args.running_job_id,
    )
    print(
        json.dumps(
            {
                "status": "KALAI_S1_RECOVERY_CONTINUATION_BATCH_AUTHORIZATION_VALID",
                "batch_id": manager.batch_id(args.batch_index),
                "authorization_payload_sha256": payload[SEAL_FIELD],
                "running_job_id": args.running_job_id,
                "external_api_calls": 0,
...
def self_test():
    ...
    assert manager.maximum_exposure(2) == Decimal("7.92198425")
    print("MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_AUTHORIZER_SELF_TEST_OK")
...
def main(argv=None):
    ...
    for command in ("preflight", "write", "verify"):
        ...
        if command == "write":
            ...
            child.add_argument("--ack-recovered-batch1-predecessor", action="store_true")
