#!/bin/bash
# Separately authorize, held-audit, and release one continuation-v3 batch.
...
manager=$repo/scripts/manage_massive_medical_kalai_s1_recovery_continuation_v3.py
authorizer=$repo/scripts/authorize_massive_medical_kalai_s1_recovery_continuation_batch_v3.py
sbatch_file=$repo/scripts/sbatch_massive_medical_kalai_s1_recovery_continuation_batch_v3_tillicum_h200.sbatch
expected_name=mmu_kalai_s1_r3c$(printf '%02d' "$batch_index")
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-rc-v3-$batch_id-submit-pyc
...
trap - EXIT
echo "Submitted and released one-shot continuation-v3 $batch_id as job $job_id."
