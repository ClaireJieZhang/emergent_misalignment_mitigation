#!/bin/bash
# Update and audit the final MASSIVE evaluation-only recovery; submit nothing.
...
test "$(git -C "$repo_root" rev-parse HEAD^)" = "$base_commit" || {
  echo "Evaluation-recovery commit is not a direct child of $base_commit" >&2
  exit 2
}
for path in "${required_files[@]}"; do
  test -s "$repo_root/$path" || {
    echo "Missing evaluation-recovery file: $path" >&2
    exit 2
  }
  git -C "$repo_root" cat-file -e "$expected_commit:$path" 2>/dev/null || {
    echo "Evaluation-recovery file is not committed at $expected_commit: $path" >&2
    exit 2
  }
done
test -z "$(git -C "$repo_root" status --porcelain -- "${required_files[@]}")" || {
  echo "Evaluation-recovery files differ from committed HEAD." >&2
...
echo "=== Run sealed evidence, sampler, and workflow audits ==="
...
test -s "$output/model/massive_en_benefit_pilot_infrastructure_recovery_v1/TRAIN_COMPLETE"
test -s "$output/control/infrastructure_recovery_v1/EVALUATE_STARTED"
test ! -e "$output/control/MASSIVE_EVALUATION_RECOVERY_V1_SUBMISSION_LOCK"
test ! -e "$output/control/evaluation_recovery_v1"
test ! -e "$output/evaluation/evaluation_recovery_v1"
...
export PYTHONPYCACHEPREFIX=$root/tmp/massive-eval-recovery-v1-stage-pyc
...
"$PYTHON" -m py_compile \
  ...
  scripts/summarize_massive_benefit_pilot.py \
  scripts/audit_massive_benefit_evaluation_recovery_v1.py
bash -n scripts/stage_massive_benefit_evaluation_recovery_v1_tillicum.sh
bash -n scripts/submit_massive_benefit_evaluation_recovery_v1_tillicum.sh
bash -n scripts/status_massive_benefit_evaluation_recovery_v1_tillicum.sh
bash -n scripts/sbatch_massive_benefit_evaluation_recovery_v1_tillicum_h200.sbatch
"$PYTHON" -m unittest \
...
"$PYTHON" scripts/audit_massive_benefit_evaluation_recovery_v1.py \
  verify-preflight \
...
for step in 15 30 60 90 150; do
...
"$PYTHON" scripts/sample_massive_structured_generations.py \
  ...
  --prompt_file "$output/data/dev/prompts.json" \
  --output_dir "$root/tmp/massive-eval-recovery-v1-preflight-unused" \
  --structured_constraint_profile const_tree_v2 \
  --max_new_tokens 256 --max_context 2048 --seed 8172026 \
  --preflight_only
REMOTE

echo "MASSIVE evaluation-recovery staging passed. No Slurm job was submitted."
echo 'Cumulative request-bound maximum: 164 H200-minutes / $2.460.'
echo "Explicit exact-once dispatch command (not run):"
echo "  ssh $TILLICUM_HOST 'cd $REMOTE_REPO_ROOT && scripts/submit_massive_benefit_evaluation_recovery_v1_tillicum.sh recover-evaluation --ack-original-max-cost-usd 2.925'"
