#!/usr/bin/env python3
"""Fail-closed sampler for the sequential MASSIVE/medical composition protocol.
...
The command has intentionally few degrees of freedom::
    ...
    sample_massive_medical_union_composition_exploratory_sequential_confirmation_v1.py \
      --phase benefit|medical \
      --protocol-manifest /absolute/path/to/manifest.json \
      --output-root /fresh/or/exact-resume/root \
      --device cuda:0

The sealed manifest supplies the four model identities, adapter roots, prompt
artifacts, method registry, and generation constants.  ``benefit`` generates a
fresh same-backend paired base and all three registered methods on the sealed
360-row MASSIVE subset.  Only a separately released ``medical`` phase generates
the three composition methods on the official 16 x 5 medical bank.  There is no
method, q, seed, temperature, prompt-count, or token-budget override.
...
def sequential_sampler_static_contract():
    """Return the output-independent scientific/runtime sampler contract."""
    probe = cache_equivalence_probe_static_contract()
    body = {
        "schema_version": SCHEMA_VERSION,
        "protocol_id": PROTOCOL_ID,
        "generation_protocol": GENERATION_PROTOCOL,
        "timing_protocol": TIMING_PROTOCOL,
        "phase_order": ["benefit", "medical"],
        "phases": {
            "benefit": {
                "domain": "massive",
                "rows": BENEFIT_PROFILE["rows"],
                "samples_per_row": BENEFIT_PROFILE["n_samples"],
                "sample_count_per_stream": BENEFIT_PROFILE["rows"],
                "stream_order": [
                    "pi_base",
                    *[method["method_id"] for method in METHODS],
                ],
                "paired_base_generated": True,
                "future_h200_minutes_cap": 65,
            },
            "medical": {
                "domain": "medical",
                "rows": MEDICAL_PROFILE["rows"],
                "samples_per_row": MEDICAL_PROFILE["n_samples"],
                "sample_count_per_stream": (
                    MEDICAL_PROFILE["rows"] * MEDICAL_PROFILE["n_samples"]
                ),
                "stream_order": [method["method_id"] for method in METHODS],
                "paired_base_generated": False,
                "future_h200_minutes_cap": 95,
            },
        },
        "tuple_rng": {
            "algorithm": "sha256_canonical_json_first_8_bytes_63bit",
            "parts": ["seed", "method_id", "question_id", "sample_index"],
            "seed": GENERATION_SEED,
            "phase_or_job_order_in_seed": False,
        },
        "model_execution_backend": INDEPENDENT_MODEL_BACKEND,
        "model_roles": list(INDEPENDENT_MODEL_ORDER),
        "scientific_adapter_switching_used": False,
        "cache_equivalence_probe_protocol": CACHE_EQUIVALENCE_PROBE_PROTOCOL,
        "cache_equivalence_probe_contract_sha256": probe["contract_sha256"],
        "probe_prompt_artifact": BENEFIT_PROFILE["artifact"],
        "structured_constraint_profile": STRUCTURED_PROFILE,
        "structured_fallback_allowed": False,
        "scientific_cli_overrides_allowed": [],
        "preflight_writes_output_root": False,
        "medical_requires_separate_phase": True,
    }
...
def selection_digest(question_id):
    material = PROTOCOL_ID + "\0benefit360\0" + question_id
    return sha256_bytes(material.encode("utf-8"))


def load_benefit_selection(protocol):
    relative = "benefit/selection.json"
    path = os.path.join(protocol["root"], "benefit", "selection.json")
    payload, _ = load_json_regular(path, "sequential benefit selection")
    body = verify_seal(payload, OUTPUT_SEAL_FIELD, "sequential benefit selection")
    expected_keys = {
        "schema_version",
        "protocol_id",
        "algorithm",
        ...
        "rank_records_sha256",
        "rank_records",
        "selected_question_ids_source_order",
    }
    if (
        set(body) != expected_keys
        or body.get("schema_version") != SCHEMA_VERSION
        or body.get("protocol_id") != PROTOCOL_ID
        or body.get("algorithm") != BENEFIT_SELECTION_ALGORITHM
        or body.get("ranking_material") != BENEFIT_SELECTION_RANKING_MATERIAL
        or body.get("tie_breaker") != "question_id_ascending"
        or body.get("source_rows") != 600
        or body.get("selected_rows") != BENEFIT_PROFILE["rows"]
        or body.get("selection_is_prompt_id_only") is not True
        or body.get("answers_or_outcomes_opened_before_selection") is not False
    ):
        raise ValueError("sequential benefit selection registry differs")
    records = body.get("rank_records")
    source_order = body.get("selected_question_ids_source_order")
    if (
        not isinstance(records, list)
        or len(records) != BENEFIT_PROFILE["rows"]
        or not isinstance(source_order, list)
        or len(source_order) != BENEFIT_PROFILE["rows"]
        or len(set(source_order)) != BENEFIT_PROFILE["rows"]
        or any(
            not isinstance(question_id, str) or not question_id
            for question_id in source_order
        )
    ):
        raise ValueError("sequential benefit selection rows differ")
    observed_ranked = []
    for row in records:
        question_id = row.get("question_id") if isinstance(row, dict) else None
        rank_sha256 = row.get("rank_sha256") if isinstance(row, dict) else None
        if (
            not isinstance(row, dict)
            or set(row) != {"rank_sha256", "question_id"}
            or not isinstance(question_id, str)
            or not question_id
            or rank_sha256 != selection_digest(question_id)
        ):
            raise ValueError("sequential benefit rank record differs")
        observed_ranked.append((rank_sha256, question_id))
    if observed_ranked != sorted(observed_ranked):
        raise ValueError("sequential benefit rank records are not in rank order")
    ranked_ids = [question_id for _, question_id in observed_ranked]
    if (
        len(set(ranked_ids)) != BENEFIT_PROFILE["rows"]
        or set(ranked_ids) != set(source_order)
        or sha256_bytes(canonical_bytes(ranked_ids)) != BENEFIT_RANKED_IDS_SHA256
        or sha256_bytes(canonical_bytes(source_order))
        != BENEFIT_SOURCE_ORDER_IDS_SHA256
        or sha256_bytes(canonical_bytes(records)) != BENEFIT_RANK_RECORDS_SHA256
        or body.get("ranked_selected_question_ids_sha256")
        != BENEFIT_RANKED_IDS_SHA256
        or body.get("selected_question_ids_source_order_sha256")
        != BENEFIT_SOURCE_ORDER_IDS_SHA256
        or body.get("rank_records_sha256") != BENEFIT_RANK_RECORDS_SHA256
    ):
        raise ValueError("sequential benefit selection hashes differ")
    manifest_selection = protocol["body"].get("selection")
    expected_manifest_selection = {
        "artifact": relative,
        "payload_sha256": payload[OUTPUT_SEAL_FIELD],
        "algorithm": BENEFIT_SELECTION_ALGORITHM,
        "ranking_material": BENEFIT_SELECTION_RANKING_MATERIAL,
        "source_rows": 600,
        "selected_rows": BENEFIT_PROFILE["rows"],
        "selection_is_prompt_id_only": True,
        "answers_or_outcomes_opened_before_selection": False,
        "ranked_selected_question_ids_sha256": BENEFIT_RANKED_IDS_SHA256,
        "selected_question_ids_source_order_sha256": BENEFIT_SOURCE_ORDER_IDS_SHA256,
        "rank_records_sha256": BENEFIT_RANK_RECORDS_SHA256,
    }
    if manifest_selection != expected_manifest_selection:
        raise ValueError("manifest benefit selection binding differs")
    return {
        "path": path,
        "file_sha256": sha256_file(path),
        "payload_sha256": payload[OUTPUT_SEAL_FIELD],
        "question_ids": source_order,
        "ranked_question_ids": ranked_ids,
    }


def expected_generation_registry(probe_prompt_binding):
    return {
        "panel_order": ["A", "B1", "B2", "B3"],
        ...
        "sequential_sampler_static_contract_sha256": (
            sequential_sampler_static_contract()["contract_sha256"]
        ),
        ...
        "probe": {
            "protocol": CACHE_EQUIVALENCE_PROBE_PROTOCOL,
            "static_contract_sha256": cache_equivalence_probe_static_contract()[
                "contract_sha256"
            ],
            "required_for_each_gpu_phase": True,
            "probe_prompt_binding": probe_prompt_binding,
        },
        "benefit": {
            "role": BENEFIT_PROFILE["role"],
            "massive_rows": BENEFIT_PROFILE["rows"],
            "n_samples": BENEFIT_PROFILE["n_samples"],
            "temperature": BENEFIT_PROFILE["temperature"],
            "max_new_tokens": BENEFIT_PROFILE["max_new_tokens"],
            "max_context": BENEFIT_PROFILE["max_context"],
            "structured_constraint_profile": STRUCTURED_PROFILE,
            "arbitrary_structural_whitespace": False,
            "truncation": False,
            "streams": ["pi_base", *[method["method_id"] for method in METHODS]],
        },
        "medical": {
            "role": MEDICAL_PROFILE["role"],
            "n_prompts": MEDICAL_PROFILE["rows"],
            "n_samples_per_prompt": MEDICAL_PROFILE["n_samples"],
            "samples_per_method": (
                MEDICAL_PROFILE["rows"] * MEDICAL_PROFILE["n_samples"]
            ),
            "temperature": MEDICAL_PROFILE["temperature"],
            "seed": GENERATION_SEED,
            "max_new_tokens": MEDICAL_PROFILE["max_new_tokens"],
            "max_context": MEDICAL_PROFILE["max_context"],
            "profile": MEDICAL_PROFILE["sampling_profile"],
            "required_finish_reason": "stop",
            "truncation": False,
            "streams": [method["method_id"] for method in METHODS],
            "paired_base_generated": False,
        },
        "paired_base": {
            "model_name": "pi_base",
...
def expected_budget_registry():
    return {
        ...
        "benefit": {
            "planning_formula": (
                "1.20*(source_setup_seconds+6*"
                "source_four_stream_smoke_seconds+60)"
            ),
...
def load_protocol_manifest(path, audit_models=True):
    ...
    if os.path.basename(path) != "manifest.json":
        raise ValueError("sequential protocol manifest must be named manifest.json")
    payload, raw = load_json_regular(path, "sequential protocol manifest")
    body = verify_seal(payload, MANIFEST_SEAL_FIELD, "sequential protocol manifest")
    expected_body_keys = {
        "schema_version",
        "protocol_id",
        "created_at",
    ...
    if (
        ...
        or not body["created_at"]
        or not isinstance(contract, dict)
        or contract.get("exploratory_only") is not True
        or contract.get("confirmatory_claim") is not False
        or contract.get("all_prior_stop_decisions_remain_terminal_and_immutable")
        is not True
        or contract.get("no_posthoc_method_threshold_seed_subset_or_profile_selection")
        is not True
        or contract.get("no_automatic_continuation") is not True
    ):
        raise ValueError("sequential protocol identity/status differs")
    if body.get("methods") != list(METHODS):
        raise ValueError("sequential method registry differs")
    if body.get("budget") != expected_budget_registry():
        raise ValueError("sequential budget/runtime registry differs")
    generation = body.get("generation")
    if not isinstance(generation, dict):
        raise ValueError("sequential generation registry is missing")
    probe = generation.get("probe")
    probe_binding = (
        probe.get("probe_prompt_binding") if isinstance(probe, dict) else None
    )
    if (
        not isinstance(probe_binding, dict)
        or set(probe_binding)
        != {"artifact", "index", "question_id", "prompt_sha256"}
        or probe_binding.get("artifact") != BENEFIT_PROFILE["artifact"]
        or probe_binding.get("index") != 0
        or not isinstance(probe_binding.get("question_id"), str)
        or not probe_binding["question_id"]
        or HEX64_RE.fullmatch(str(probe_binding.get("prompt_sha256"))) is None
        or generation != expected_generation_registry(probe_binding)
    ):
    ...
    if not isinstance(copied, dict) or set(copied) != expected_copied:
        raise ValueError("sequential copied-artifact bindings differ")
    for relative in sorted(expected_copied):
        binding = copied[relative]
        artifact_path = os.path.join(root, *relative.split("/"))
        require_file_binding(artifact_path, binding, f"sequential {relative}")
        if not isinstance(binding, dict):
            raise ValueError(f"sequential artifact binding differs: {relative}")
        if binding.get("path") != relative:
            raise ValueError(f"sequential artifact path binding differs: {relative}")
        if relative in {
            "benefit/selection.json",
            "benefit/prompts.json",
            "benefit/answers.json",
        }:
            artifact_payload, _ = load_json_regular(
                artifact_path, f"sealed sequential {relative}"
            )
            verify_seal(
                artifact_payload, OUTPUT_SEAL_FIELD, f"sealed sequential {relative}"
            )
            if (
                binding.get("payload_seal_field") != OUTPUT_SEAL_FIELD
                or binding.get("payload_sha256")
                != artifact_payload[OUTPUT_SEAL_FIELD]
            ):
                raise ValueError(
                    f"sequential artifact payload binding differs: {relative}"
                )
        elif relative == "medical/prompts.json":
            if binding.get("byte_identical") is not True:
                raise ValueError("sequential medical prompt copy is not byte-identical")
        elif relative == "historical/A_judgments.json":
            if (
                binding.get("byte_identical") is not True
                or binding.get("payload_seal_field") != OUTPUT_SEAL_FIELD
            ):
                raise ValueError("sequential historical-A copy binding differs")
    ...
    if (
        ...
        raise ValueError("sequential model panel differs")
    audited_references = {}
    for role in PANEL_ORDER:
    ...
    result["selection"] = load_benefit_selection(result)
    return result
...
def stream_plan(
    ...
    """Return the exact sequential phase plan; no stream is shared across phases."""
    if phase == "benefit":
        return [
            (method_by_id("pi_base"), benefit_profile, benefit_records),
            *[
                (dict(method), benefit_profile, benefit_records)
                for method in METHODS
            ],
        ]
    if phase == "medical":
        if medical_profile is None or medical_records is None:
            raise ValueError("medical phase requires the frozen medical prompt bank")
        return [
            (dict(method), medical_profile, medical_records)
            for method in METHODS
        ]
    raise ValueError(f"unknown sequential phase: {phase}")


def expected_phase_stream_keys(phase):
    if phase == "benefit":
        return [
            "pi_base:massive",
            *[
                f"{method['method_id']}:massive"
                for method in METHODS
            ],
        ]
    if phase == "medical":
        return [
            f"{method['method_id']}:medical"
            for method in METHODS
        ]
    raise ValueError(f"unknown sequential phase: {phase}")
...
def build_timing_record(protocol, phase, setup_seconds, streams, cache_probe):
    ...
    expected_keys = expected_phase_stream_keys(phase)
    if list(by_key) != expected_keys:
        raise ValueError("sequential timing stream order/registry differs")
    phase_budget = protocol["body"]["budget"].get(phase)
    if phase_budget != expected_budget_registry()[phase]:
        raise ValueError("sequential phase budget binding differs")
    return {
        "schema_version": SCHEMA_VERSION,
        ...
        "stream_registry": expected_keys,
        "streams": by_key,
        "phase_budget_binding": phase_budget,
        "paired_base_generation_recorded_separately": phase == "benefit",
        "runtime_projection_owned_by_sequential_evaluator": True,
    }


def audit_timing_record(path, protocol, phase, streams):
    payload, _ = load_json_regular(path, "sequential phase timings")
    body = verify_seal(payload, OUTPUT_SEAL_FIELD, "sequential phase timings")
    ...
    expected_keys = {
        ...
        "post_generation_artifact_audit_seconds",
        "runtime_versions",
    }
    if set(body) != expected_keys:
        raise ValueError("sequential phase timing schema differs")
    expected_prefix = {
        "schema_version": SCHEMA_VERSION,
        "protocol": TIMING_PROTOCOL,
        "protocol_id": PROTOCOL_ID,
        ...
        "stream_registry": expected_phase_stream_keys(phase),
        "phase_budget_binding": protocol["body"]["budget"][phase],
        "paired_base_generation_recorded_separately": phase == "benefit",
        "runtime_projection_owned_by_sequential_evaluator": True,
    }
    for key, value in expected_prefix.items():
        if body.get(key) != value:
            raise ValueError(f"sequential phase timing differs on {key}")
    runtime = body.get("runtime_versions")
    if runtime != {
        "torch": PINNED_TORCH_VERSION,
        "transformers": PINNED_TRANSFORMERS_VERSION,
        "peft": PINNED_PEFT_VERSION,
        ...
        raise ValueError("sequential phase timing runtime pins differ")
    if (
        isinstance(body.get("setup_seconds"), bool)
        or not isinstance(body.get("setup_seconds"), (int, float))
        or body["setup_seconds"] < 0
    ):
        raise ValueError("sequential phase timing has invalid setup metadata")
    ...
    if (
        ...
        raise ValueError("sequential timing fixed-overhead decomposition differs")
    expected_streams = {
        f"{stream['method_id']}:{stream['domain']}": {
            key: value
            for key, value in stream.items()
    ...
    if (
        list(expected_streams) != expected_phase_stream_keys(phase)
        or body.get("streams") != expected_streams
    ):
        raise ValueError(
            "sequential timing stream measurements differ from sample shards"
        )
    return body
...
def build_cpu_preflight_result(protocol, phase, runtime, base_snapshot, grammar, plan):
    stream_registry = [
        f"{method['method_id']}:{profile['domain']}"
        for method, profile, _ in plan
    ]
    if stream_registry != expected_phase_stream_keys(phase):
        raise ValueError("sequential CPU preflight stream registry differs")
    ...
    return {
        ...
        "sequential_sampler_contract": sequential_sampler_static_contract(),
        ...
        "stream_plan": [
            {
                ...
                "domain": profile["domain"],
                "rows": len(records),
                "n_samples_per_row": profile["n_samples"],
                "sample_count": len(records) * profile["n_samples"],
            }
        ...
        "benefit_selection_binding": {
            ...
            "rows": len(selection["question_ids"]),
            "question_ids_sha256": BENEFIT_SOURCE_ORDER_IDS_SHA256,
        },
        "probe_prompt_binding": protocol["body"]["generation"]["probe"][
            "probe_prompt_binding"
        ],
...
def run_phase(args):
    ...
    if args.preflight_only:
        ...
        _, _, grammar = load_tokenizer_and_grammar(benefit_profile, base_snapshot)
        result = build_cpu_preflight_result(
            protocol, args.phase, runtime, base_snapshot, grammar, plan
        )
    ...
    with exclusive_phase_lock(output_root, args.phase) as phase_root:
        run_manifest = {
            ...
            "cache_equivalence_probe_contract_sha256": (
                cache_equivalence_probe_static_contract()["contract_sha256"]
            ),
            "phase_budget_binding": protocol["body"]["budget"][args.phase],
            "stream_registry": expected_phase_stream_keys(args.phase),
            "probe_prompt_binding": protocol["body"]["generation"]["probe"][
                "probe_prompt_binding"
            ],
            "benefit_selection_binding": {
                "artifact": "benefit/selection.json",
                "payload_sha256": protocol["selection"]["payload_sha256"],
                "question_ids_sha256": BENEFIT_SOURCE_ORDER_IDS_SHA256,
                "rows": BENEFIT_PROFILE["rows"],
            },
            "streams": [
...
def main(argv=None):
    ...
    parser.add_argument("--phase", required=True, choices=("benefit", "medical"))
