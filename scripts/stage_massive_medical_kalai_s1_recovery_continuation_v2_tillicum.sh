#!/bin/bash
# Clone and CPU-stage the recovered-batch-2 continuation-v2 controller.
...
[[ $# -eq 0 ]] || {
  echo 'Usage: stage_massive_medical_kalai_s1_recovery_continuation_v2_tillicum.sh' >&2
  exit 2
}
...
branch=${REMOTE_BRANCH:-claire/massive-medical-kalai-s1-batch2-recovery-continuation-v2}
...
recovery_repo=$root/projects/subliminal-mitigate-mmu-kalai-s1-batch2-result-recovery-v1
...
test ! -e "$repo"
test ! -e "$output"
if compgen -G "$logs/massive_medical_kalai_s1_recovery_continuation_v2_batch_*" >/dev/null; then
  echo 'Continuation-v2 Slurm log namespace is not fresh.' >&2
  exit 4
fi
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-recovery-continuation-v2-stage-pyc
...
python -m unittest tests.test_massive_medical_kalai_s1_recovery_continuation_v2
python scripts/manage_massive_medical_kalai_s1_recovery_continuation_v2.py self-test
python scripts/authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py self-test
python scripts/sample_massive_medical_kalai_s1_recovery_continuation_batch_v2.py --self-test
python scripts/evaluate_massive_medical_kalai_s1_recovery_continuation_batch_v2.py --self-test
python scripts/assemble_massive_medical_kalai_s1_recovery_continuation_v2.py --self-test

python scripts/manage_massive_medical_kalai_s1_recovery_continuation_v2.py stage \
...
for batch_index in $(seq 3 7); do
  python scripts/manage_massive_medical_kalai_s1_recovery_continuation_v2.py preflight \
    --output-root "$output" --repo-root "$repo" --batch-index "$batch_index"
done
python scripts/authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py preflight \
...
echo KALAI_S1_RECOVERY_CONTINUATION_V2_CPU_STAGED_NO_GPU_OR_API_AUTHORITY
REMOTE

echo 'Kalai s=1 continuation-v2 batches 3--7 CPU-staged.'
