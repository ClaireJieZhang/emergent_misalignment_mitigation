#!/bin/bash
# Read-only status for the one-shot exploratory independent-model recovery v3.

set -euo pipefail

[[ $# -eq 0 ]] || {
  echo 'Usage: scripts/status_massive_medical_union_composition_exploratory_independent_model_recovery_v3_tillicum.sh' >&2
  exit 2
}
...
if [[ -e $control/INDEPENDENT_MODEL_RECOVERY_CPU_PREFLIGHT.json ]]; then
...
if [[ -e $control/INDEPENDENT_MODEL_RECOVERY_JOB.json ]]; then
  job_id=$(python -c 'import json,sys; print(json.load(open(sys.argv[1]))["job_id"])' "$control/INDEPENDENT_MODEL_RECOVERY_JOB.json")
  ...
  if [[ -e $control/INDEPENDENT_MODEL_RECOVERY_RESULT.json ]]; then
    ...
    case $state in
      ...
      FAILED*|CANCELLED*|TIMEOUT*|OUT_OF_MEMORY*|NODE_FAIL*|PREEMPTED*)
        ...
        if [[ -e $control/STOPPED_independent_model_recovery ]]; then
          cat "$control/STOPPED_independent_model_recovery"
        else
  ...
  elif [[ -e $control/STOPPED_independent_model_recovery ]]; then
    state=$(printf '%s\n' "$accounting" | awk -F'|' -v id="$job_id" '$1 == id {print $3}')
    case $state in
      FAILED*|CANCELLED*|TIMEOUT*|OUT_OF_MEMORY*|NODE_FAIL*|PREEMPTED*)
        python "$auditor" audit-stopped-terminal
        echo 'RECOVERY_STOPPED_TERMINAL; RETRY IS FORBIDDEN'
        ;;
      COMPLETED)
        echo 'RECOVERY_STOP_SENTINEL_WITH_COMPLETED_ACCOUNTING'
        exit 6
        ;;
      *)
        echo "RECOVERY_STOP_SEALED_AWAITING_TERMINAL state=${state:-ACCOUNTING_PENDING}"
        ;;
    esac
  else
    state=$(printf '%s\n' "$accounting" | awk -F'|' -v id="$job_id" '$1 == id {print $3}')
