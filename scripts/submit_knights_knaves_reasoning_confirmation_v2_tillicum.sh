#!/bin/bash
# Submit the single held-first, 30-minute K&K v2 evaluation-only job.
...
if [[ "$#" -ne 3 || "$1" != confirmation || "$2" != --ack-max-cost-usd || "$3" != 0.45 ]]; then
  cat >&2 <<'EOF'
Usage:
  scripts/submit_knights_knaves_reasoning_confirmation_v2_tillicum.sh confirmation --ack-max-cost-usd 0.45

This submits exactly one no-requeue, non-array H200 job capped at 30 minutes.
Maximum new cost is $0.45.  Together with the v1 released allocation caps,
the cumulative released maximum is 180 H200-minutes ($2.70), below the
existing immutable 240-minute/$3.60 ceiling.  It cannot train, create extra
adapters, submit medical unions, or run quorum.
EOF
  exit 2
fi
...
"$ENV_ROOT/bin/python" scripts/audit_knights_knaves_confirmation_v2_workflow.py verify-prep \
  --repo-root "$REPO" --v1-root "$V1_ROOT" --v2-data-root "$V2_DATA" \
  --prep-file "$PREP"

# A first submission must start before any v2 inference result or scientific
# decision exists.  This prevents stale/manual outputs from being accepted by
# the resumable samplers after authorization is recorded.
for sentinel in \
  GO_KK_V2_SEALED_FINAL STOPPED_KK_V2_NO_GO \
  GO_KK_V2_BENEFIT_UNIONS STOPPED_KK_V2_FINAL; do
  if [[ -e "$CONTROL/$sentinel" || -L "$CONTROL/$sentinel" ]]; then
    echo "Refusing preexisting K&K v2 decision sentinel: $CONTROL/$sentinel" >&2
    exit 3
  fi
done
if [[ -L "$EVAL" ]]; then
  echo "Refusing symlinked K&K v2 evaluation path: $EVAL" >&2
  exit 3
fi
if [[ -e "$EVAL" ]]; then
  test -d "$EVAL" || {
    echo "Refusing unsafe preexisting K&K v2 evaluation path: $EVAL" >&2
    exit 3
  }
  if [[ -n "$(find "$EVAL" -mindepth 1 -print -quit)" ]]; then
    echo "Refusing preexisting K&K v2 evaluation outputs under $EVAL" >&2
    exit 3
  fi
fi

if ! mkdir "$LOCK" 2>/dev/null; then
  echo "A K&K v2 submission attempt already owns $LOCK" >&2
  exit 3
fi
printf 'created_at=%s\nrepo_commit=%s\n' \
...
if [[ -e "$AUTH" || -e "$JOBS" || -e "$ATTEMPT" || -e "$SUBMITTED" || -e "$RELEASED" ]]; then
  echo "K&K v2 submission state already exists; refusing duplicate allocation" >&2
  exit 3
fi

"$ENV_ROOT/bin/python" scripts/audit_knights_knaves_confirmation_v2_workflow.py write-authorization \
  --repo-root "$REPO" --v1-root "$V1_ROOT" --v2-data-root "$V2_DATA" \
  --prep-file "$PREP" --ack-max-cost-usd 0.45 --output-file "$AUTH"
chmod 0400 "$AUTH"
...
echo "Submitted and released K&K v2 evaluation job $job"
echo 'Hard maximum: 30 H200-minutes = $0.45.'
echo "No training, extra adapter, medical union, or quorum job exists."
