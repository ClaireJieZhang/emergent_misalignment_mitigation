#!/bin/bash
# Authorize, held-audit, and release exactly one Kalai s=1 completion batch.
...
control=$control_root/batches/$batch_id
logs=$root/outputs/logs
authorizer=$repo/scripts/authorize_massive_medical_kalai_s1_completion_batch_v1.py
sbatch_file=$repo/scripts/sbatch_massive_medical_kalai_s1_completion_batch_v1_tillicum_h200.sbatch
expected_name=mmu_kalai_s1_b$(printf '%02d' "$batch_index")
expected_time=01:00:00

cd "$repo"
test -z "$(git status --porcelain)"
test -s "$control_root/CPU_STAGE.json"
test -s "$control_root/COMPLETION_BATCH_PLAN.json"
test ! -e "$control"
...
if (( batch_index < 7 )); then
  for future_index in $(seq "$((batch_index + 1))" 7); do
    ...
    test ! -e "$control_root/batches/$future_id"
...
trap - EXIT
echo "Submitted and released one-shot Kalai s=1 $batch_id as job $job_id."
