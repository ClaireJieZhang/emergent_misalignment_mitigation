#!/bin/bash
# External-judge finalizer for sealed medical recovery v2. No GPU path.
...
usage() {
  echo 'Usage: scripts/finalize_massive_medical_union_wave1_medical_recovery_v2_tillicum.sh external-judge --ack-max-api-cost-usd 0.75' >&2
  exit 2
}
...
RECOVERY_EVAL_ROOT=$OLD_EVAL_ROOT/medical_recovery_v2
...
export PYTHONPYCACHEPREFIX=$TILLICUM_ROOT/tmp/mmu-medical-recovery-v2-finalize-pyc
unset HF_TOKEN HUGGINGFACE_HUB_TOKEN HUGGING_FACE_HUB_TOKEN
unset WANDB_API_KEY ANTHROPIC_API_KEY COHERE_API_KEY GOOGLE_API_KEY

python scripts/audit_massive_medical_union_medical_recovery_v2.py audit-gpu
...
# This validates every exact request and the aggregate cap before any API call.
...
flock -n 9 || {
  echo 'Another recovery-v2 external-judge finalizer holds the active lock.' >&2
  exit 5
}
...
if [[ -s "$GATE_ROOT/GO_MASSIVE_UNION_WAVE1" ]]; then
  ...
  echo "Recovery-v2 component summarizer produced no terminal decision (status $summary_status)." >&2
  (( summary_status != 0 )) || summary_status=6
  exit "$summary_status"
fi

trap - EXIT
echo 'FINAL_EVALUATION_COMPLETE: see the sealed recovery-v2 Wave-1 component summary.'
