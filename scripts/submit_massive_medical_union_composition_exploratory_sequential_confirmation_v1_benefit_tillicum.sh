...
if compgen -G "$logs/massive_medical_union_composition_exploratory_sequential_confirmation_v1_submit_recovery_v3_benefit_*" >/dev/null; then echo 'Benefit log namespace is not fresh.' >&2; exit 4; fi
module load conda/Miniforge3-25.3.1-3
conda activate "$root/envs/subliminal-mitigate-py311"
export PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=$root/tmp/mmu-seq-benefit-v3-submit-pyc
...
raw_job=$(sbatch --parsable --hold --export=NONE --job-name=mmu_seq_benefit_v3 "$batch")
