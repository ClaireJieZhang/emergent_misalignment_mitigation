#!/bin/bash
# Explicit capped judge + final gate after evaluation recovery. No Slurm/GPU.
...
usage() {
  echo 'Usage: scripts/finalize_massive_medical_union_wave2_evaluation_recovery_v1_tillicum.sh external-judge --ack-max-api-cost-usd 0.50' >&2
  exit 2
}
...
cd "$REPO_ROOT"
test -s "$CONTROL_ROOT/GPU_EVAL_RECOVERY_COMPLETE"
test -s "$EVAL_ROOT/GPU_EVAL_RECOVERY_MANIFEST.json"
...
test ! -e "$CONTROL_ROOT/STOPPED_evaluate_recovery_v1"
...
export PYTHONPYCACHEPREFIX=$TILLICUM_ROOT/tmp/mmu-wave2-eval-recovery-v1-finalize-pyc
unset HF_TOKEN HUGGINGFACE_HUB_TOKEN HUGGING_FACE_HUB_TOKEN
unset WANDB_API_KEY ANTHROPIC_API_KEY COHERE_API_KEY GOOGLE_API_KEY

python "$AUDITOR" audit-gpu
...
# Any prior interruption record remains immutable evidence; it is not removed.
...
python scripts/summarize_massive_union_components.py gate \
  ...
  --model_manifest "pi_B2=$RECOVERED_MANIFEST_ROOT/pi_B2/MODEL_MANIFEST.json" \
  --model_manifest "pi_B3=$RECOVERED_MANIFEST_ROOT/pi_B3/MODEL_MANIFEST.json" \
...
python "$AUDITOR" write-final-decision
...
trap - EXIT
echo 'WAVE2_EVALUATION_RECOVERY_FINAL_COMPLETE: see sealed wrapper and component summary.'
