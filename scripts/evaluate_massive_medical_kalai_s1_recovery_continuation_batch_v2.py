#!/usr/bin/env python3
"""Audit and seal one continuation-v2 batch result."""
...
manager = _load("_kalai_s1_rc_v2_manager_for_evaluator", "manage_massive_medical_kalai_s1_recovery_continuation_v2.py")
authorizer = _load("_kalai_s1_rc_v2_authorizer_for_evaluator", "authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py")
runtime = _load("_kalai_s1_rc_v2_runtime_for_evaluator", "sample_massive_medical_kalai_s1_recovery_continuation_batch_v2.py")


def _load_authorization(output_root, repo_root, batch_index):
    path = authorizer.authorization_path(output_root, batch_index)
    payload = manager.load_json(path, "continuation-v2 authorization")
    body = manager.verify_seal(payload, "continuation-v2 authorization")
    if body != authorizer.expected_body(output_root, repo_root, batch_index, body.get("created_at")):
        raise ValueError("continuation-v2 authorization differs")
...
def _phase_outputs(audit):
    ...
    for phase in manager.PHASES:
        ...
        generation = manager.load_json(generation_path, f"continuation-v2 {phase} generation")
        manager.verify_seal(generation, f"continuation-v2 {phase} generation")
        timing_path = Path(audit["phases"][phase]["timing"])
        timing = manager.load_json(timing_path, f"continuation-v2 {phase} timing")
        manager.verify_seal(timing, f"continuation-v2 {phase} timing")
...
def _expected_result_body(output_root, repo_root, batch_index, *, slurm_job_id, elapsed_seconds, audit):
    ...
    combined = manager.load_json(combined_path, "continuation-v2 combined timing")
    manager.verify_seal(combined, "continuation-v2 combined timing")
    ...
    return {
        ...
        "status": "MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_V2_BATCH_COMPLETE",
        "batch_valid": True,
...
def evaluate(args):
    ...
    manager._write_new(path, result, "continuation-v2 batch result")
...
def load_and_verify_result(output_root, repo_root, batch_index, *, audit_generation):
    ...
    if audit_generation:
        ...
        if body != expected:
            raise ValueError("continuation-v2 result artifacts differ")
    return result


def self_test():
    assert Decimal(3600) * manager.H200_HOURLY_USD / Decimal(3600) == manager.BATCH_CAP_USD
    assert manager.PROTOCOL_ID.endswith("recovery_continuation_v2")
    source = Path(__file__).read_text(encoding="utf-8")
    assert "return runtime.generation_audit(output, context, batch_index)" in source
    print("MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_V2_EVALUATOR_SELF_TEST_OK")
