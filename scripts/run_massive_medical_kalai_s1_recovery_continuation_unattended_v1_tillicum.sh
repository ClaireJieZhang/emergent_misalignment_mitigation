...
[[ $# -eq 0 ]] || {
  echo 'Usage: run_massive_medical_kalai_s1_recovery_continuation_unattended_v1_tillicum.sh' >&2
  exit 2
}
...
manager=$repo/scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v1.py
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-unattended-v1-controller-pyc
...
python "$manager" begin --output-root "$output" --repo-root "$repo"
...
for batch_index in 3 4 5 6 7; do
  ...
  python "$manager" authorize-batch \
  ...
  [[ $active_job =~ ^[0-9]+$ ]]
  echo "UNATTENDED_V1_WAITING batch=$batch_index job=$active_job"
...
python "$manager" assemble --output-root "$output" --repo-root "$repo"
active_batch=complete
trap - EXIT
echo KALAI_S1_RECOVERY_CONTINUATION_UNATTENDED_V1_COMPLETE
