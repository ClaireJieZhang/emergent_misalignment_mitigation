#!/bin/bash
# CPU-only staging for the one-shot exploratory independent-model recovery v3.

set -euo pipefail
umask 077

[[ $# -eq 0 ]] || {
  echo 'Usage: scripts/stage_massive_medical_union_composition_exploratory_independent_model_recovery_v3_tillicum.sh' >&2
  exit 2
}
...
REMOTE_BRANCH=${REMOTE_BRANCH:-claire/capability-quorum-secure-code-composition-exploratory-independent-model-recovery-v3}
...
branch=$3
expected_commit=$4
repo=$root/projects/subliminal-mitigate-mmu-composition-exploratory-independent-model-recovery-v3
output=$root/outputs/massive_medical_union_composition_exploratory_independent_model_recovery_v3
...
probe_recovery_v2_repo=$root/projects/subliminal-mitigate-mmu-composition-exploratory-smoke-probe-recovery-v2
probe_recovery_v2_output=$root/outputs/massive_medical_union_composition_exploratory_smoke_probe_recovery_v2
...
test -d "$probe_recovery_v2_repo"
test -d "$probe_recovery_v2_output"
test -f "$env_root/.ready"
if compgen -G "$root/outputs/logs/massive_medical_union_composition_exploratory_independent_model_recovery_v3_*" >/dev/null; then
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-composition-independent-model-recovery-v3-stage-pyc
...
python scripts/audit_massive_medical_union_composition_exploratory_independent_model_recovery_v3.py write-prep
preflight=$output/control/INDEPENDENT_MODEL_RECOVERY_CPU_PREFLIGHT.json
...
python scripts/audit_massive_medical_union_composition_exploratory_independent_model_recovery_v3.py audit-preflight
python scripts/audit_massive_medical_union_composition_exploratory_independent_model_recovery_v3.py write-staged
python scripts/audit_massive_medical_union_composition_exploratory_independent_model_recovery_v3.py audit-staged
REMOTE

echo 'Exploratory independent-model recovery v3 staged and CPU-sealed. No job or API call was made.'
