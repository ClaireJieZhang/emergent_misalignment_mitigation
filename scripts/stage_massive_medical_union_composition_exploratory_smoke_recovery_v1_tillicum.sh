#!/bin/bash
# CPU-only staging for the one-shot exploratory smoke infrastructure recovery.

set -euo pipefail
umask 077

[[ $# -eq 0 ]] || {
  echo 'Usage: scripts/stage_massive_medical_union_composition_exploratory_smoke_recovery_v1_tillicum.sh' >&2
  exit 2
}
...
REMOTE_BRANCH=${REMOTE_BRANCH:-claire/capability-quorum-secure-code-composition-exploratory-smoke-recovery-v1}
...
if compgen -G "$root/outputs/logs/massive_medical_union_composition_exploratory_smoke_recovery_v1_*" >/dev/null; then
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-composition-smoke-recovery-v1-stage-pyc
...
python scripts/audit_massive_medical_union_composition_exploratory_smoke_recovery_v1.py write-prep
preflight=$output/control/SMOKE_RECOVERY_CPU_PREFLIGHT.json
...
python scripts/audit_massive_medical_union_composition_exploratory_smoke_recovery_v1.py audit-preflight
python scripts/audit_massive_medical_union_composition_exploratory_smoke_recovery_v1.py write-staged
python scripts/audit_massive_medical_union_composition_exploratory_smoke_recovery_v1.py audit-staged
REMOTE

echo 'Exploratory smoke recovery v1 staged and CPU-sealed. No job or API call was made.'
