#!/bin/bash
# Read-only status/audit for judge recovery v5.

set -euo pipefail
umask 077
ulimit -c 0

[[ $# -eq 0 ]] || { echo 'Usage: status_..._judge_recovery_v5_tillicum.sh' >&2; exit 2; }
...
if [[ ! -d $repo || ! -d $output ]]; then
  echo 'JUDGE_RECOVERY_V5_NOT_STAGED'
  exit 0
fi
...
if [[ -f $output/control/FINAL_RESULT.json ]]; then
  ...
  echo 'JUDGE_RECOVERY_V5_CANARY_LOCKED_OR_RUNNING'
elif [[ -f $output/control/CANARY_LOCK_OWNER.json ]]; then
  python "$auditor" audit-lock --stage canary
  echo 'JUDGE_RECOVERY_V5_CANARY_LOCKED_BEFORE_AUTHORIZATION_OR_RUNNING'
else
  python "$auditor" audit-staged
  echo 'JUDGE_RECOVERY_V5_CPU_STAGED_AWAITING_SEPARATE_ONE_CALL_AUTHORIZATION'
fi
