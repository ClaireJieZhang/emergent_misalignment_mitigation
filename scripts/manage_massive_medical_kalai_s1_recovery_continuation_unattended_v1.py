...
v3_manager = _load(
    "_kalai_s1_rc_v3_manager_for_unattended_v1",
    "manage_massive_medical_kalai_s1_recovery_continuation_v3.py",
)
v3_authorizer = _load(
    "_kalai_s1_rc_v3_authorizer_for_unattended_v1",
    "authorize_massive_medical_kalai_s1_recovery_continuation_batch_v3.py",
)
v3_evaluator = _load(
    "_kalai_s1_rc_v3_evaluator_for_unattended_v1",
    "evaluate_massive_medical_kalai_s1_recovery_continuation_batch_v3.py",
)
v3_assembler = _load(
    "_kalai_s1_rc_v3_assembler_for_unattended_v1",
    "assemble_massive_medical_kalai_s1_recovery_continuation_v3.py",
)

PROTOCOL_ID = "massive_medical_kalai_s1_recovery_continuation_unattended_v1"
EXPECTED_REPO_LEAF = "subliminal-mitigate-mmu-kalai-s1-recovery-continuation-unattended-v1"
...
_NEW_FILES = (
    "configs/pipelines/massive_medical_kalai_s1_recovery_continuation_unattended_v1.yaml",
    "docs/massive_medical_kalai_s1_recovery_continuation_unattended_v1_protocol.md",
    "scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v1.py",
    ...
    "tests/test_massive_medical_kalai_s1_recovery_continuation_unattended_v1.py",
)
...
def _expected_plan_body(v3: dict, output_root: Path | str) -> dict:
    summaries = [
        {
            "batch_index": index,
...
def _load_anchor_from_plan(plan_body: dict, *, require_fresh: bool = False) -> dict:
    v3 = _audit_v3_anchor(
...
def _load_workflow(output_root: Path | str, repo_root: Path | str, *, require_fresh_v3: bool = False):
    ...
    v3 = _load_anchor_from_plan(plan_body, require_fresh=require_fresh_v3)
    if plan_body != _expected_plan_body(v3, output):
...
def stage(args):
    ...
    _assert_disjoint(output, repo, args.v3_output_root, args.v3_repo_root)
    require_clean(repo, "unattended controller")
    v3 = _audit_v3_anchor(args.v3_output_root, args.v3_repo_root, require_fresh=True)
    plan = seal(_expected_plan_body(v3, output.resolve()))
    ...
    print(
        json.dumps(
            {
                "status": "KALAI_S1_UNATTENDED_V1_CPU_STAGED_AND_AUTHORIZED",
                "batch_indices": list(BATCH_INDICES),
                "maximum_gpu_jobs": 5,
                "maximum_new_candidate_attempts": MAXIMUM_NEW_CANDIDATE_ATTEMPTS,
                "total_new_h200_cap_usd": float(TOTAL_NEW_CAP_USD),
    ...
    final = seal(
        {
            ...
            "status": "KALAI_S1_UNATTENDED_V1_COMPLETE_AND_ASSEMBLED",
            "unattended_plan": binding(output / "control" / PLAN_NAME, plan),
...
def self_test():
    ...
    print("MASSIVE_MEDICAL_KALAI_S1_UNATTENDED_V1_SELF_TEST_OK")
