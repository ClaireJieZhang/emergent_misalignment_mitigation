...
[[ $# -eq 0 ]] || {
  echo 'Usage: stage_massive_medical_kalai_s1_recovery_continuation_unattended_v2_tillicum.sh' >&2
  exit 2
}
...
branch=${REMOTE_BRANCH:-claire/massive-medical-kalai-s1-unattended-v2}
...
test -s "$v1_output/control/UNATTENDED_PLAN.json"
test -s "$v1_output/control/CPU_STAGE.json"
test -s "$v1_output/control/SEQUENCE_AUTHORIZATION.json"
test -s "$v1_output/control/UNATTENDED_INVOCATION.json"
test -s "$v1_output/control/SEQUENCE_STOPPED"
test -s "$v1_output/control/batches/batch_03/CHILD_AUTHORITY.json"
test "$(find "$v1_output" -type f | wc -l)" = 6
test -z "$(find "$v1_output" -type l -print -quit)"
...
test ! -e "$repo"
test ! -e "$output"
test -z "$(tmux list-sessions -F '#S' 2>/dev/null | awk '$0=="kalai-s1-unattended-v2"')"
if compgen -G "$logs/massive_medical_kalai_s1_recovery_continuation_unattended_v2*" >/dev/null; then
  echo 'Unattended-v2 log namespace is not fresh.' >&2
  exit 4
fi
...
export PYTHONPYCACHEPREFIX=$root/tmp/mmu-kalai-s1-unattended-v2-stage-pyc
...
python -m py_compile scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py
python -m unittest tests.test_massive_medical_kalai_s1_recovery_continuation_unattended_v2
python scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py self-test
(
  unset CONDA_PREFIX CONDA_DEFAULT_ENV CONDA_PROMPT_MODIFIER CONDA_SHLVL
  unset CONDA_EXE CONDA_PYTHON_EXE _CE_CONDA _CE_M
  "$env_root/bin/python" scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py self-test
)

python scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py stage \
  --output-root "$output" --repo-root "$repo" \
  --v3-output-root "$v3_output" --v3-repo-root "$v3_repo" \
  --v1-output-root "$v1_output" --v1-repo-root "$v1_repo"
python scripts/manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py stage \
  --output-root "$output" --repo-root "$repo" \
  --v3-output-root "$v3_output" --v3-repo-root "$v3_repo" \
  --v1-output-root "$v1_output" --v1-repo-root "$v1_repo"
...
test ! -e "$v1_output/control/batches/batch_03/SACCT.tsv"
test -z "$(squeue -h -u "$USER" -n mmu_kalai_s1_r3c03,mmu_kalai_s1_r3c04,mmu_kalai_s1_r3c05,mmu_kalai_s1_r3c06,mmu_kalai_s1_r3c07)"
echo KALAI_S1_RECOVERY_CONTINUATION_UNATTENDED_V2_CPU_STAGED_VALID
REMOTE

echo 'Kalai s=1 unattended-v2 CPU-staged with sealed standing authority.'
