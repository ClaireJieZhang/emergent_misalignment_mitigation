#!/usr/bin/env python3
"""Fail-closed auditor for the one-shot exploratory smoke probe recovery.
...
PROTOCOL_ID = (
    "massive_medical_union_composition_exploratory_protocol_smoke_probe_recovery_v2"
)
...
DIRECT_PARENT_COMMIT = "8b5f8db2d54b6fc7da3d54c1cb024e950201d879"
DIRECT_PARENT_TREE = "41c142e2cfedb9b0e32fae529143029d702d72f3"
...
PREFLIGHT_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_CPU_PREFLIGHT.json"
STAGED_FILE = CONTROL_ROOT / "STAGED"
LOCK_ROOT = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_SUBMISSION_LOCK"
ATTEMPT_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_SUBMISSION_ATTEMPT.tsv"
JOB_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_JOB.json"
AUTH_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_AUTHORIZED_MAX_COST_USD_0.225.json"
SUBMITTED_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_SUBMITTED"
RELEASE_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_RELEASE_AUTHORIZED"
RESULT_FILE = CONTROL_ROOT / "SMOKE_PROBE_RECOVERY_RESULT.json"
STOP_FILE = CONTROL_ROOT / "STOPPED_smoke_probe_recovery"
...
JOB_NAME = "mmu_cmpx_probe_rec_v2"
LOG_PREFIX = "massive_medical_union_composition_exploratory_smoke_probe_recovery_v2"
...
PRIOR_ACTUAL_SECONDS = ORIGINAL_ACTUAL_SECONDS + PROBE_RECOVERY_V1_ACTUAL_SECONDS
PRIOR_ACTUAL_H200_MINUTES = PRIOR_ACTUAL_SECONDS / 60.0
PRIOR_ACTUAL_COST_USD = ORIGINAL_ACTUAL_COST_USD + PROBE_RECOVERY_V1_ACTUAL_COST_USD
RECOVERY_CAP_MINUTES = 15
RECOVERY_CAP_COST_USD = 0.225
ACTUAL_PLUS_RECOVERY_CAP_COST_USD = 0.25150
...
def audit_probe_recovery_v1_incident():
    if (
        PROBE_RECOVERY_V1_REPO.is_symlink()
    ...
    return {
        ...
        "commit": DIRECT_PARENT_COMMIT,
        "tree": DIRECT_PARENT_TREE,
...
def audit_local_weight_resolver(snapshot=None):
    ...
    body = {
        "protocol": (
            ...
            "local_snapshot_resolver_smoke_probe_recovery_v2"
        ),
...
def budget_registry():
    return {
        ...
        "probe_recovery_v2_h200_minutes_max": RECOVERY_CAP_MINUTES,
        "probe_recovery_v2_gpu_cost_usd_max": RECOVERY_CAP_COST_USD,
        "prior_actual_plus_probe_recovery_v2_cap_h200_minutes": (
            PRIOR_ACTUAL_H200_MINUTES + RECOVERY_CAP_MINUTES
        ),
        "prior_actual_plus_probe_recovery_v2_cap_gpu_cost_usd": (
...
def expected_probe_v2_static_contract():
    top_level_keys = {
        ...
        "cache_object_count", "cache_tensor_storage_sets_checked",
        "cache_tensor_storages_disjoint", "model_compute_dtype",
        "attention_implementation", "comparison_dtype", "repeatability_gate",
        "diagnostic_policy", "diagnostic_top_k", "vocab_size",
        "repeatability", "cached_vs_full_prefix_diagnostics", "probe_seconds",
    }
    ...
    body = {
        "schema_version": 1,
        "protocol": "massive_medical_union_composition_cache_equivalence_probe_v2",
...
def audit_probe_v2_static_contract(value):
    if value != expected_probe_v2_static_contract():
        raise ValueError("sampler cache-equivalence probe-v2 static contract differs")
    return value
...
def load_preflight():
    ...
    audit_probe_v2_static_contract(payload.get("cache_equivalence_probe_contract"))
    expected_keys = {
        *expected_scalar,
        "runtime",
        "schema_sha256",
...
def staged_bytes(prep, preflight):
    return (
        ...
        f"probe_recovery_v1_commit={DIRECT_PARENT_COMMIT}\n"
        f"probe_recovery_v1_job_id={PROBE_RECOVERY_V1_JOB_ID}\n"
        "probe_recovery_v1_stop_preserved=true\n"
        "stage_submitted_jobs=0\n"
        "prior_failed_actual_gpu_cost_usd=0.02650\n"
        "probe_recovery_v2_gpu_h200_minutes_max=15\n"
        "probe_recovery_v2_gpu_cost_usd_max=0.225\n"
        "prior_actual_plus_probe_recovery_v2_cap_gpu_cost_usd=0.25150\n"
        "training=false\n"
        "external_api_calls=0\n"
        "recovery_confirmation_submission_code_present=false\n"
    ).encode("utf-8")
...
def command_assert_submit_ready(_args):
    audit_staged()
    expected = {"PREP.json", "SMOKE_PROBE_RECOVERY_CPU_PREFLIGHT.json", "STAGED"}
    observed = set()
...
def job_pointer():
    ...
    if (
        ...
        or body.get("workflow_id") != WORKFLOW_ID
        or body.get("stage") != "smoke_probe_recovery"
...
def expected_recovery_control_inventory(phase):
    ...
    if phase in ("running", "result"):
        expected.add("SMOKE_PROBE_RECOVERY_RELEASE_AUTHORIZED")
    if phase == "result":
        expected.add("SMOKE_PROBE_RECOVERY_RESULT.json")
...
def audit_recovery_log_inventory(phase, job_id):
    ...
    if phase not in ("held", "running", "result"):
...
def command_verify_job(args):
    ...
    print("SMOKE_PROBE_RECOVERY_RUNNING_AUDIT_OK")
...
def terminal_accounting():
    ...
    return {
        ...
        "prior_failed_actual_gpu_cost_usd": PRIOR_ACTUAL_COST_USD,
        "prior_actual_plus_probe_recovery_v2_actual_gpu_cost_usd": (
