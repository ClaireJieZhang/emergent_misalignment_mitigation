#!/usr/bin/env python3
"""Authorize exactly one fresh continuation-v3 batch in indices 3--7."""
...
manager = _load(
    "_kalai_s1_rc_v3_manager_for_authorizer",
...
def _plan_summary(plan_body, batch_index):
    ...
    if not isinstance(batches, list) or [item.get("batch_index") for item in batches] != list(BATCH_INDICES):
        raise ValueError("continuation-v3 batch inventory differs")
...
def predecessor_binding(output_root, repo_root, plan_body, batch_index):
    ...
    evaluator = _load(
        f"_kalai_s1_rc_v3_evaluator_for_predecessor_{batch_index}",
    ...
    if os.path.lexists(stopped):
        raise ValueError("preceding continuation-v3 batch is stopped")
    path = control_root(output_root, batch_index - 1) / "RESULT.json"
    return {"kind": "preceding_recovery_continuation_v3_batch_result", "artifact": manager.binding(path, previous)}
...
def preflight_predecessor(args):
    ...
    print(json.dumps({"status": "KALAI_S1_RECOVERY_CONTINUATION_V3_PREDECESSOR_PREFLIGHT_VALID", "batch_id": manager.batch_id(args.batch_index), "continuation_plan_payload_sha256": plan[manager.SEAL_FIELD], "predecessor_kind": predecessor["kind"], "row_count": summary["row_count"], "gpu_jobs": 0, "external_api_calls": 0}, sort_keys=True))
    return predecessor
...
def verify_authorization(args):
    ...
    print(json.dumps({"status": "KALAI_S1_RECOVERY_CONTINUATION_V3_BATCH_AUTHORIZATION_VALID", "batch_id": manager.batch_id(args.batch_index), "authorization_payload_sha256": payload[manager.SEAL_FIELD], "running_job_id": args.running_job_id, "external_api_calls": 0}, sort_keys=True))
    return payload
...
def self_test():
    ...
    print("MASSIVE_MEDICAL_KALAI_S1_RECOVERY_CONTINUATION_V3_AUTHORIZER_SELF_TEST_OK")
