...
export PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=$root/tmp/mmu-seq-medical-v3-submit-pyc
...
if compgen -G "$logs/massive_medical_union_composition_exploratory_sequential_confirmation_v1_submit_recovery_v3_medical_*" >/dev/null; then echo 'Medical log namespace is not fresh.' >&2; exit 4; fi
...
raw_job=$(sbatch --parsable --hold --export=NONE --job-name=mmu_seq_medical_v3 "$batch")
