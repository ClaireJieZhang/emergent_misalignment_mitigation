#!/bin/bash
# Permanent single-entry canary or continuation for the panel-diagnostic judge.
...
[[ $mode == canary || $mode == continuation ]] || {
  echo 'Usage: finalize_massive_medical_panel_diagnostic_judge_v1_tillicum.sh canary|continuation EXACT_ACK_FLAGS' >&2
  exit 2
}
shift
[[ -n ${OPENAI_API_KEY:-} ]] || {
...
if [[ $run_code -ne 0 ]]; then
  ...
  if [[ -f $failure ]]; then
    ...
    echo "PANEL_DIAGNOSTIC_JUDGE_${mode^^}_TERMINAL_FAILURE_NO_RESTART"
  elif [[ -f $started ]]; then
    python "$runner" status --manifest "$manifest"
    echo "PANEL_DIAGNOSTIC_JUDGE_${mode^^}_RUN_STARTED_NO_RESTART_OR_SECOND_ENTRY"
  else
    echo "PANEL_DIAGNOSTIC_JUDGE_${mode^^}_PREENTRY_FAILURE_NO_API_CALL" >&2
  fi
  exit "$run_code"
fi
...
if [[ $mode == canary ]]; then
  ...
  echo PANEL_DIAGNOSTIC_JUDGE_CONTINUATION_COMPLETE
fi
