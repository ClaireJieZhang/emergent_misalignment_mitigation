#!/usr/bin/env python3
"""Prepare the immutable, explicitly exploratory MASSIVE composition protocol.

This is deliberately a post-Wave-2 protocol.  It preserves the terminal Wave-2
STOP, accepts only its exact 69/70 near-miss shape, and copies the prospectively
frozen Wave-3-v1 data artifacts byte-for-byte into a fresh namespace.  It does
not load a model, submit Slurm work, call an API, or authorize Wave 3.
"""
...
EXPECTED_SOURCE_INVENTORY = set(COPIED_PATHS)
...
PILOT_ROOT = TILLICUM_ROOT + "/outputs/massive_medical_union_pilot_v1"
DEFAULT_SOURCE_PROTOCOL = PILOT_ROOT + "/protocol/wave3_composition_v1"
DEFAULT_WAVE2_DECISION = (
    PILOT_ROOT + "/control/wave2_eval_recovery_v1/WAVE2_FINAL_DECISION.json"
)
DEFAULT_SCORE_ROOT = PILOT_ROOT + "/evaluation/wave2_eval_recovery_v1/massive/scores"
DEFAULT_OUTPUT_ROOT = (
    TILLICUM_ROOT
...
def inventory(root, exclude=(MANIFEST_NAME,)):
    ...
    if os.path.islink(root) or not os.path.isdir(root):
        raise ValueError("Protocol tree root is not a regular non-symlink directory")
    result = []
...
def parse_named(values, expected, option):
    result = {}
    for value in values:
        if "=" not in value:
            raise ValueError(f"{option} must be NAME=PATH: {value!r}")
        name, path = (item.strip() for item in value.split("=", 1))
        if name not in expected or name in result or not path:
            raise ValueError(f"Invalid or duplicate {option}: {value!r}")
        result[name] = os.path.abspath(path)
    if set(result) != set(expected):
        raise ValueError(f"{option} names must be exactly {list(expected)}")
    return result
...
def _source_inventory_map(manifest):
    entries = manifest.get("file_inventory")
    if not isinstance(entries, list):
        raise ValueError("Source Wave-3 manifest file_inventory is not a list")
    result = {}
    for entry in entries:
        if (
            not isinstance(entry, dict)
            or set(entry) != {"path", "size_bytes", "sha256"}
            or entry.get("path") in result
        ):
            raise ValueError("Source Wave-3 manifest inventory is malformed")
        result[entry["path"]] = entry
    return result


def _validate_source_registries(manifest):
    if manifest.get("methods") != expected_method_registry():
        raise ValueError("Source Wave-3 method registry differs")
    if manifest.get("generation") != expected_source_generation_registry():
        raise ValueError("Source Wave-3 generation registry differs")
    if manifest.get("gates") != expected_gate_registry():
        raise ValueError("Source Wave-3 gate registry differs")
    if manifest.get("budget") != expected_budget_registry():
        raise ValueError("Source Wave-3 budget registry differs")
    if manifest.get("judge") != expected_judge_registry():
        raise ValueError("Source Wave-3 judge registry differs")
...
def audit_source_protocol(root):
    root = os.path.abspath(root)
    manifest_path = os.path.join(root, "protocol_manifest.json")
    manifest, raw = load_json(manifest_path, "source Wave-3 manifest")
    verify_seal(manifest, "manifest_payload_sha256", "source Wave-3 manifest")
    if (
        manifest.get("schema_version") != 1
        or manifest.get("protocol_id") != SOURCE_PROTOCOL_ID
        or manifest.get("subset_contract_revision") != SOURCE_SUBSET_REVISION
        or manifest.get("prospective") is not True
    ):
        raise ValueError("Source Wave-3 protocol identity differs")
    _validate_source_registries(manifest)
    source_inventory = _source_inventory_map(manifest)
    if set(source_inventory) != EXPECTED_SOURCE_INVENTORY:
        raise ValueError("Source Wave-3 artifact paths differ")
    actual = {item["path"]: item for item in inventory(root, exclude=("protocol_manifest.json",))}
    if actual != source_inventory:
        raise ValueError("Source Wave-3 artifact inventory differs from its manifest")
    validate_subset_artifacts(root)
    return manifest, {
        **stable_path_binding(
            manifest_path, manifest, "manifest_payload_sha256"
        ),
        "protocol_root": root,
        "protocol_id": SOURCE_PROTOCOL_ID,
        "subset_contract_revision": SOURCE_SUBSET_REVISION,
        "artifact_inventory": list(source_inventory.values()),
    }


def _require_id_records(payload, key, rows, description):
...
def validate_subset_artifacts(root):
    ...
    _, smoke_ids = _require_id_records(
        smoke_prompts, "prompts", EXPECTED_SMOKE_ROWS, "smoke prompts"
    )
    _, smoke_answer_ids = _require_id_records(
        smoke_answers, "answers", EXPECTED_SMOKE_ROWS, "smoke answers"
    )
    confirmation_records, confirmation_ids = _require_id_records(
        confirmation_prompts,
        "prompts",
        EXPECTED_CONFIRMATION_ROWS,
        "confirmation prompts",
    )
    _, confirmation_answer_ids = _require_id_records(
        confirmation_answers,
        "answers",
        EXPECTED_CONFIRMATION_ROWS,
        "confirmation answers",
    )
    medical_records, medical_ids = _require_id_records(
    ...
    if smoke_ids != smoke_answer_ids or confirmation_ids != confirmation_answer_ids:
        raise ValueError("Source Wave-3 prompt/answer orders differ")
    if medical_ids != [f"medical_official16_{index:02d}" for index in range(16)]:
        raise ValueError("Source Wave-3 medical question order differs")
    if smoke_prompts["meta"].get("contains_gold_labels") is not False:
        raise ValueError("Smoke prompt artifact exposes gold labels")
    if confirmation_prompts["meta"].get("contains_gold_labels") is not False:
        raise ValueError("Confirmation prompt artifact exposes gold labels")
    if medical.get("meta", {}).get("contains_answers") is not False:
        raise ValueError("Medical prompt artifact exposes answers")
    for record in confirmation_records + medical_records:
        if not isinstance(record.get("prompt"), str) or not record["prompt"]:
            raise ValueError("Source Wave-3 prompt record is invalid")
    return confirmation_ids


def audit_wave2_terminal(path):
    ...
    if (
        stable_path_binding(summary_path)["file_sha256"]
        != summary_binding.get("file_sha256")
        ...
        raise ValueError("Wave-2 component summary binding differs")
    ...
    if (
        ...
        or [key for key, passed in checks.items() if passed is not True]
        != [SOLE_WAVE2_FAILURE]
    ):
    ...
    if (
        stable_path_binding(sentinel_path)["file_sha256"]
        != sentinel_binding.get("file_sha256")
        ...
        raise ValueError("Wave-2 STOP sentinel binding differs")
    ...
    return summary, {
        **stable_path_binding(path, decision, "payload_sha256"),
        ...
        "checks_total": EXPECTED_WAVE2_CHECKS,
        "checks_true": EXPECTED_WAVE2_CHECKS - 1,
...
def audit_source_medical_generation(source, expected_name):
    ...
    for index, sample in enumerate(samples):
        expected_cell = (
            f"medical_official16_{index // 5:02d}",
            index % 5,
        )
...
def audit_medical_evidence(summary):
    ...
    arms = {name: _medical_arm_metrics(by_model[name]) for name in MODELS}
...
def _manifest_inventory_map(entries, description):
    ...
    for entry in entries:
        ...
        if (
            ...
            raise ValueError(f"{description} inventory entry differs")
        result[path] = {
            "path": path,
...
def audit_model_manifest(name, binding):
    ...
    actual = stable_path_binding(path, payload, seal_field)
    if (
        actual["file_sha256"] != binding.get("file_sha256")
        or payload[seal_field] != binding.get("payload_sha256")
        or body.get("model_name") != name
    ):
        raise ValueError(f"{name} model-manifest binding differs")
    fingerprint = body.get("adapter_fingerprint")
    artifacts = body.get("adapter_artifacts")
    adapter_dir = body.get("adapter_dir")
    if (
        not re.fullmatch(r"[0-9a-f]{64}", str(fingerprint))
        ...
        raise ValueError(f"{name} adapter binding differs")
    ...
    expected_inventory = _manifest_inventory_map(body.get("inventory"), name)
    actual_inventory = {
        item["path"]: item
        for item in inventory(adapter_dir, exclude=("MODEL_MANIFEST.json", "TRAIN_COMPLETE"))
    }
...
def audit_model_panel(summary):
    candidates = summary.get("candidates")
    if not isinstance(candidates, dict) or set(candidates) != set(PANEL):
        raise ValueError("Wave-2 candidate set differs")
    references = {}
    for name in PANEL:
        binding = candidates[name].get("model_manifest")
        if not isinstance(binding, dict):
            raise ValueError(f"Wave-2 summary lacks {name} model manifest")
        references[name] = audit_model_manifest(name, binding)
    fingerprints = [references[name]["model_fingerprint"] for name in PANEL]
    if len(set(fingerprints)) != len(PANEL):
        raise ValueError("Composition panel fingerprints are not distinct")
    identities = {
        (references[name]["base_model"], references[name]["base_model_revision"])
        for name in PANEL
    }
    if len(identities) != 1 or None in next(iter(identities)):
        raise ValueError("Composition panel base identity differs")
    b_dataset = {
        (
    ...
    if len(b_dataset) != 1 or None in next(iter(b_dataset)):
        raise ValueError("Good-medical replicas do not bind one B dataset")
    base_model, base_revision = next(iter(identities))
    return {
        "panel_order": list(PANEL),
        "base": {
            "model_name": "pi_base",
            "model_path": "BASE",
...
def audit_score(path, name, summary):
    payload, _ = load_json(path, f"{name} full-test score")
    body = verify_seal(payload, "payload_sha256", f"{name} full-test score")
    ...
    if (
        ...
        raise ValueError(f"{name} full-test score contract differs")
    ...
    if any(not isinstance(value, str) for value in ids) or len(set(ids)) != len(ids):
        raise ValueError(f"{name} score question IDs differ")
    summary_binding = (
        summary.get("base") if name == "pi_base" else summary["candidates"][name].get("score")
    )
    if (
        not isinstance(summary_binding, dict)
        or os.path.abspath(path) != os.path.abspath(summary_binding.get("path", ""))
        or sha256_file(path) != summary_binding.get("file_sha256")
        or payload["payload_sha256"] != summary_binding.get("payload_sha256")
    ):
        raise ValueError(f"{name} score is not the terminal-summary artifact")
    return payload, body, {row["question_id"]: row for row in tasks}


def write_direct_comparators(staging, score_paths, confirmation_ids, summary, panel):
    results = {}
    expected_fingerprints = {
        "pi_base": "BASE",
        **{
            name: panel["references"][name]["model_fingerprint"] for name in PANEL
        },
    }
    question_ids_sha = sha256_bytes(canonical_bytes(confirmation_ids))
    for name in MODELS:
        path = score_paths[name]
        source_payload, source_body, by_id = audit_score(path, name, summary)
        if source_body["meta"].get("model_fingerprint") != expected_fingerprints[name]:
            raise ValueError(f"{name} score/model fingerprint differs")
        missing = [question_id for question_id in confirmation_ids if question_id not in by_id]
        if missing:
            raise ValueError(f"{name} score misses confirmation question IDs")
        tasks = [by_id[question_id] for question_id in confirmation_ids]
        body = {
            "schema_version": 1,
            "protocol_id": PROTOCOL_ID,
            "model_name": name,
            "source_score": stable_path_binding(
                path, source_payload, "payload_sha256"
            ),
            "selection": {
                "rows": EXPECTED_CONFIRMATION_ROWS,
                "question_ids": confirmation_ids,
                "question_ids_sha256": question_ids_sha,
            },
            "tasks": tasks,
        }
        payload = seal(body, COMPARATOR_SEAL)
        relative = f"direct_confirmation/{name}.json"
        output = os.path.join(staging, relative)
        ...
        results[name] = {
            ...
            "source_score": body["source_score"],
            "rows": EXPECTED_CONFIRMATION_ROWS,
            "question_ids_sha256": question_ids_sha,
        }
    return {
        "models": results,
        "base_model": "pi_base",
        "panel_mean_models": list(PANEL),
        "rows": EXPECTED_CONFIRMATION_ROWS,
        "question_ids_sha256": question_ids_sha,
    }


def copy_source_artifacts(source_root, staging):
    bindings = {}
    for relative in COPIED_PATHS:
        source = os.path.join(source_root, relative)
        raw = read_regular_bytes(source, f"source {relative}")
        destination = os.path.join(staging, relative)
        atomic_bytes(destination, raw)
        if read_regular_bytes(destination, f"copied {relative}") != raw:
            raise RuntimeError(f"Copied protocol artifact differs: {relative}")
        bindings[relative] = {
...
def runtime_projection_registry(planning_envelope):
    ...
    return {
        ...
        "medical_all_three_methods_selected_tokens_bound": 3 * selected_tokens_per_method,
...
def exploratory_judge_registry(source_judge):
    registry = dict(source_judge)
    registry.update(
        {
...
def generation_registry(source_generation):
    registry = dict(source_generation)
    registry["paired_base"] = {
        "model_name": "pi_base",
    ...
    registry["direct_confirmation_comparators"] = {
...
def build_protocol(
    ...
    wave2_final_decision,
    output_root,
    direct_score_paths,
    created_at=None,
):
    source_manifest, source_binding = audit_source_protocol(source_protocol_root)
    summary, wave2_binding, risk_check, planning_envelope = audit_wave2_terminal(
        wave2_final_decision
    )
    realized = wave2_binding["realized_composition_preregistration"]
    if (
        os.path.abspath(realized["manifest_path"]) != source_binding["path"]
        or realized["manifest_file_sha256"] != source_binding["file_sha256"]
        or realized["manifest_payload_sha256"] != source_binding["payload_sha256"]
    ):
        raise ValueError("Wave-2 realized protocol is not the copied Wave-3 source")
    panel = audit_model_panel(summary)
    for name in PANEL:
        if (
    ...
    confirmation_ids = validate_subset_artifacts(source_protocol_root)
    output_root = os.path.abspath(output_root)
    if os.path.lexists(output_root):
        raise ValueError(f"Refusing to replace existing exploratory protocol: {output_root}")
    parent = os.path.dirname(output_root)
    os.makedirs(parent, exist_ok=True)
    if os.path.islink(parent) or not os.path.isdir(parent):
        raise ValueError("Exploratory protocol parent is unsafe")
    staging = tempfile.mkdtemp(prefix="composition-exploratory-v1-", dir=parent)
    try:
        copied = copy_source_artifacts(source_protocol_root, staging)
        comparators = write_direct_comparators(
            staging, direct_score_paths, confirmation_ids, summary, panel
        )
        body = {
            ...
            "source_wave2_terminal": wave2_binding,
            "source_wave3_protocol": source_binding,
            "exploratory_execution_risk_check": risk_check,
            "methods": source_manifest["methods"],
            "generation": generation_registry(source_manifest["generation"]),
            "gates": source_manifest["gates"],
            "budget": source_manifest["budget"],
            "judge": exploratory_judge_registry(source_manifest["judge"]),
            "runtime_projection": runtime_projection_registry(planning_envelope),
            "model_panel": panel,
            "direct_confirmation": comparators,
            "copied_artifacts": copied,
            "file_inventory": inventory(staging),
        }
        payload = seal(body, MANIFEST_SEAL)
        atomic_json(os.path.join(staging, MANIFEST_NAME), payload)
        if os.path.lexists(output_root):
            raise ValueError("Exploratory protocol output appeared during preparation")
...
def default_direct_scores():
    return {
        name: os.path.join(DEFAULT_SCORE_ROOT, f"massive_en_test__{name}.json")
        for name in MODELS
    }
...
def main(argv=None):
    ...
    parser.add_argument(
        "--source-wave3-protocol-root", default=DEFAULT_SOURCE_PROTOCOL
    )
    parser.add_argument("--wave2-final-decision", default=DEFAULT_WAVE2_DECISION)
    parser.add_argument("--output-root", default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument(
        "--direct-score",
        action="append",
        default=[],
        help="NAME=sealed full-test score; repeat for pi_base,A,B1,B2,B3",
    )
    args = parser.parse_args(argv)
    scores = (
        parse_named(args.direct_score, MODELS, "--direct-score")
        if args.direct_score
        else default_direct_scores()
    )
    old_umask = os.umask(0o077)
    try:
        manifest = build_protocol(
            args.source_wave3_protocol_root,
            args.wave2_final_decision,
            args.output_root,
            scores,
        )
    finally:
        os.umask(old_umask)
    print(f"Prepared exploratory composition protocol: {manifest}")
    print("Wave-2 STOP preserved; no GPU, Slurm, API, or Wave-3 action performed.")
