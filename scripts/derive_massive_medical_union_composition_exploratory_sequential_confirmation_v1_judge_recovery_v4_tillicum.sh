#!/bin/bash
# Idempotent CPU-only derivation after a completed 240-call judge stage.

set -euo pipefail
umask 077
ulimit -c 0

[[ $# -eq 0 ]] || { echo 'Usage: derive_..._judge_recovery_v4_tillicum.sh' >&2; exit 2; }
[[ -z ${OPENAI_API_KEY:-} ]] || {
...
echo "JUDGE_RECOVERY_V4_CPU_DERIVATION_COMPLETE scientific_exit=$scientific_code"
