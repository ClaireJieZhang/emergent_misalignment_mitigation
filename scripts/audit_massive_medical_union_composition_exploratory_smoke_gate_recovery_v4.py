...
SCHEMA_VERSION = 1
WORKFLOW_ID = (
    "massive_medical_union_composition_exploratory_smoke_gate_recovery_v4"
)
PROTOCOL_ID = (
    "massive_medical_union_composition_exploratory_smoke_gate_recovery_v4"
)
...
DIRECT_PARENT_COMMIT = SOURCE_COMMIT
DIRECT_PARENT_TREE = SOURCE_TREE
BRANCH = (
    "claire/capability-quorum-secure-code-composition-exploratory-"
...
FROZEN_MODIFIED_FILE_SIZE = {
    MODIFIED_FILES[0]: 113364,
    MODIFIED_FILES[1]: 48982,
    MODIFIED_FILES[2]: 36204,
}
ADDED_FILES = (
...
EXECUTABLE_FILES = tuple(
    path for path in (*MODIFIED_FILES, *ADDED_FILES) if path.startswith("scripts/")
)
REGULAR_FILES = tuple(
    path for path in (*MODIFIED_FILES, *ADDED_FILES) if not path.startswith("scripts/")
)
...
def audit_repository():
    ...
    if len(observed) != len(expected) or set(observed) != set(expected):
        raise ValueError("gate-recovery commit differs from exact 3M+5A allowlist")
    files = {}
    for relative in (*MODIFIED_FILES, *ADDED_FILES):
        ...
        if (
            ...
            or actual_mode != (0o755 if relative in EXECUTABLE_FILES else 0o644)
        ):
            raise ValueError(f"gate-recovery mode differs for {relative}")
        digest = sha256_file(path)
        if relative in FROZEN_MODIFIED_FILE_SHA256 and (
            digest != FROZEN_MODIFIED_FILE_SHA256[relative]
            or path.stat().st_size != FROZEN_MODIFIED_FILE_SIZE[relative]
        ):
            raise ValueError(f"frozen evaluator bytes differ for {relative}")
        files[relative] = {
...
def load_fixed_evaluator():
    path = require_regular(REPO_ROOT / MODIFIED_FILES[0], "fixed evaluator")
    if (
        path.stat().st_size != FROZEN_MODIFIED_FILE_SIZE[MODIFIED_FILES[0]]
        or sha256_file(path) != FROZEN_MODIFIED_FILE_SHA256[MODIFIED_FILES[0]]
    ):
    ...
    name = "mmu_composition_smoke_gate_recovery_v4_fixed_evaluator"
...
def staged_bytes(prep):
    return (
        ...
        f"fixed_evaluator_sha256={FROZEN_MODIFIED_FILE_SHA256[MODIFIED_FILES[0]]}\n"
