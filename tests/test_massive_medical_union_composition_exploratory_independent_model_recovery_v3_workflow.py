...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "independent_model_recovery_v3_workflow_test", path
    )
...
class RecoveryWorkflowTests(unittest.TestCase):
    def test_exact_scope_namespaces_and_budget(self):
        ...
        self.assertEqual(
            ...
            "5e1bb439d60410f6e6ba853a0dae80625a72f9d3",
        )
        self.assertEqual(
            recovery.DIRECT_PARENT_TREE,
            "56dd4835cabe51ff6484abf0730d8ce309aabdb5",
        )
        self.assertEqual(recovery.PROBE_RECOVERY_V1_JOB_ID, "261327")
        self.assertEqual(recovery.PROBE_RECOVERY_V2_JOB_ID, "261839")
        self.assertEqual(recovery.PROBE_RECOVERY_V2_COMMIT, recovery.DIRECT_PARENT_COMMIT)
        self.assertEqual(recovery.PROBE_RECOVERY_V2_TREE, recovery.DIRECT_PARENT_TREE)
        self.assertEqual(
            recovery.BRANCH,
            "claire/capability-quorum-secure-code-composition-exploratory-independent-model-recovery-v3",
        )
        self.assertTrue(str(recovery.REPO_ROOT).endswith("-independent-model-recovery-v3"))
        self.assertTrue(str(recovery.OUTPUT_ROOT).endswith("_independent_model_recovery_v3"))
        self.assertEqual(recovery.PRIOR_ACTUAL_SECONDS, 182)
        self.assertAlmostEqual(recovery.PRIOR_ACTUAL_H200_MINUTES, 182 / 60)
        self.assertEqual(recovery.PRIOR_ACTUAL_COST_USD, 0.04550)
        self.assertEqual(recovery.RECOVERY_CAP_MINUTES, 15)
        self.assertEqual(recovery.RECOVERY_CAP_COST_USD, 0.225)
        self.assertEqual(recovery.ACTUAL_PLUS_RECOVERY_CAP_COST_USD, 0.27050)
        self.assertEqual(
    ...
    def test_probe_v3_static_contract_is_exact_and_incidents_are_not_thresholds(self):
        contract = recovery.expected_probe_v3_static_contract()
        self.assertIs(recovery.audit_probe_v3_static_contract(contract), contract)
        self.assertEqual(
            contract["contract_sha256"],
            "b15890642418ad34f1ade97b3433ea5432ad53221a8e0b544fee29942c2cbc1d",
        )
        sampler_path = (
            ROOT / "scripts/sample_massive_medical_union_composition_exploratory_v1.py"
        )
        sampler_spec = importlib.util.spec_from_file_location(
            "independent_model_recovery_v3_sampler_contract_test", sampler_path
        )
        sampler = importlib.util.module_from_spec(sampler_spec)
        sampler_spec.loader.exec_module(sampler)
        self.assertEqual(contract, sampler.cache_equivalence_probe_static_contract())
        self.assertEqual(contract["cache_object_count"], 5)
        self.assertEqual(contract["model_object_count"], 5)
        self.assertEqual(
            contract["hard_gate"]["mode"],
            "independent_model_isolation_and_cached_execution",
        )
        self.assertFalse(
            contract["hard_gate"][
        ...
        with self.assertRaisesRegex(ValueError, "static contract differs"):
            recovery.audit_probe_v3_static_contract(drifted)

    def test_cpu_preflight_binds_snapshot_and_probe_v3_contract(self):
        payload = {
            ...
            "cache_equivalence_probe_contract": (
                recovery.expected_probe_v3_static_contract()
            ),
    ...
    def test_source_incident_hash_contract_is_exact_and_no_science(self):
        ...
        self.assertEqual(
            recovery.PROBE_RECOVERY_V2_CONTROL_FILES[
                "STOPPED_smoke_probe_recovery"
            ],
            (
                217,
                "8ab5eeb790f9364b4b06479155b6e458d4dc5f1c8613113fbb844faa2eb2d9e5",
            ),
        )
        self.assertEqual(
            recovery.PROBE_RECOVERY_V2_GENERATION_FILES,
            recovery.SOURCE_GENERATION_FILES,
        )
        self.assertEqual(
            set(recovery.PROBE_RECOVERY_V2_LOG_FILES),
            {
    ...
    def test_probe_recovery_v2_sacct_accepts_exact_terminal_row(self):
        row = (
            "261839|mmu_cmpx_probe_rec_v2|FAILED|00:01:16|00:15:00|"
            "billing=8,cpu=8,gres/gpu=1,gres/gpu:h200=1,mem=180G,node=1|"
            "billing=8,cpu=8,gres/gpu=1,gres/gpu:h200=1,mem=180G,node=1|"
            "1:0|g007"
        )
        with mock.patch.object(
            ...
            observed = recovery.probe_recovery_v2_sacct()
        self.assertEqual(observed["elapsed_seconds"], 76)
        self.assertEqual(observed["actual_gpu_cost_usd"], 0.01900)
        self.assertEqual(observed["sacct_row"], row)

    def test_future_stopped_terminal_accounting_is_bounded_and_nonzero(self):
        row = (
            "999|mmu_cmpx_ind_rec_v3|FAILED|00:01:00|00:15:00|"
            "billing=8,cpu=8,gres/gpu=1,gres/gpu:h200=1,mem=180G,node=1|"
            "billing=8,cpu=8,gres/gpu=1,gres/gpu:h200=1,mem=180G,node=1|"
            "1:0"
        )
        with mock.patch.object(
            recovery, "job_pointer", return_value={"job_id": "999"}
        ), mock.patch.object(
            recovery.subprocess, "check_output", return_value=row + "\n"
        ):
            observed = recovery.stopped_terminal_accounting()
        self.assertEqual(observed["state"], "FAILED")
        self.assertEqual(observed["elapsed_seconds"], 60)
        self.assertEqual(observed["actual_gpu_cost_usd"], 0.015)
        completed = row.replace("FAILED", "COMPLETED").replace("1:0", "0:0")
        with mock.patch.object(
            recovery, "job_pointer", return_value={"job_id": "999"}
        ), mock.patch.object(
            recovery.subprocess, "check_output", return_value=completed + "\n"
        ):
            with self.assertRaisesRegex(ValueError, "accounting differs"):
                recovery.stopped_terminal_accounting()
    ...
    def test_authorized_provenance_rejects_repo_or_protocol_drift(self):
        ...
        self.assertIn("audit_probe_recovery_v2_incident()", source)
    ...
    def test_control_and_scientific_namespaces_are_phase_exact(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(recovery, "CONTROL_ROOT", control), mock.patch.object(
                ...
                (control / "INDEPENDENT_MODEL_RECOVERY_RELEASE_AUTHORIZED").write_text("sealed")
                ...
                (control / "INDEPENDENT_MODEL_RECOVERY_RESULT.json").write_text("sealed")
    ...
    def test_lock_owner_and_submitted_records_are_exact_and_private(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            attempt.write_bytes(
                b"stage\tjob_id\nindependent_model_recovery\t123\n"
            )
    ...
    def test_shells_are_held_first_one_shot_and_have_no_confirmation_or_api(self):
        docs = (ROOT / recovery.ADDED_FILES[0]).read_text()
        ...
        self.assertIn("--ack-prior-actual-cost-usd && $3 == 0.04550", submit)
        self.assertIn("$7 == 0.27050", submit)
        self.assertIn(
            "source_commit=5e1bb439d60410f6e6ba853a0dae80625a72f9d3",
            stage,
        )
        self.assertIn(
        ...
        self.assertIn("audit-stopped-terminal", status)
        self.assertIn("RECOVERY_STOP_SEALED_AWAITING_TERMINAL", status)
        for value in ("261152", "261327", "261839", "$0.04550", "$0.27050"):
            self.assertIn(value, docs)
        self.assertIn(recovery.PROBE_V3_STATIC_CONTRACT_SHA256, docs)
        self.assertIn("five independent Transformers/PEFT model objects", docs)
        self.assertEqual(
