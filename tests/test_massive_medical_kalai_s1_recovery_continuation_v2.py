"""CPU-only regressions for recovered-batch-2 continuation v2."""
...
manager = load_script("_test_kalai_s1_rc_v2_manager", "manage_massive_medical_kalai_s1_recovery_continuation_v2.py")
authorizer = load_script("_test_kalai_s1_rc_v2_authorizer", "authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py")
runtime = load_script("_test_kalai_s1_rc_v2_runtime", "sample_massive_medical_kalai_s1_recovery_continuation_batch_v2.py")
evaluator = load_script("_test_kalai_s1_rc_v2_evaluator", "evaluate_massive_medical_kalai_s1_recovery_continuation_batch_v2.py")
assembler = load_script("_test_kalai_s1_rc_v2_assembler", "assemble_massive_medical_kalai_s1_recovery_continuation_v2.py")


class ContinuationV2PlanTests(unittest.TestCase):
    def test_exact_lineage_and_accounting(self):
        ...
        self.assertEqual(manager.RECOVERY_PROTOCOL_ID, "massive_medical_kalai_s1_batch2_result_recovery_v1")
...
class ContinuationV2WorkflowTests(unittest.TestCase):
    def test_cpu_stage_wrapper_has_no_paid_command(self):
        source = (SCRIPTS / "stage_massive_medical_kalai_s1_recovery_continuation_v2_tillicum.sh").read_text(encoding="utf-8")
    ...
    def test_first_predecessor_is_recovery_then_fresh_results(self):
        source = (SCRIPTS / "authorize_massive_medical_kalai_s1_recovery_continuation_batch_v2.py").read_text(encoding="utf-8")
        self.assertIn('"kind": "recovered_batch_2_result"', source)
        self.assertIn('"kind": "preceding_recovery_continuation_v2_batch_result"', source)
    ...
    def test_evaluator_uses_runtime_owned_audit_module(self):
        evaluation = (SCRIPTS / "evaluate_massive_medical_kalai_s1_recovery_continuation_batch_v2.py").read_text(encoding="utf-8")
        runtime_source = (SCRIPTS / "sample_massive_medical_kalai_s1_recovery_continuation_batch_v2.py").read_text(encoding="utf-8")
    ...
    def test_stopped_v2_batch_is_never_result_eligible(self):
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / manager.EXPECTED_OUTPUT_LEAF
    ...
    def test_submit_is_held_first_and_sbatch_is_one_shot(self):
        submit = (SCRIPTS / "submit_massive_medical_kalai_s1_recovery_continuation_batch_v2_tillicum.sh").read_text(encoding="utf-8")
        sbatch = (SCRIPTS / "sbatch_massive_medical_kalai_s1_recovery_continuation_batch_v2_tillicum_h200.sbatch").read_text(encoding="utf-8")
    ...
    def test_assembly_binds_both_recovered_results(self):
        source = (SCRIPTS / "assemble_massive_medical_kalai_s1_recovery_continuation_v2.py").read_text(encoding="utf-8")
