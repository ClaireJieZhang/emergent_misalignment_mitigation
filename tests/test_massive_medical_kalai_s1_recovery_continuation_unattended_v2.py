...
SCRIPT = ROOT / "scripts" / "manage_massive_medical_kalai_s1_recovery_continuation_unattended_v2.py"
spec = importlib.util.spec_from_file_location("unattended_v2_tested", SCRIPT)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)


class UnattendedV2Tests(unittest.TestCase):
    @staticmethod
    ...
    def test_runner_is_fail_closed_and_has_no_api_or_retry_path(self):
        runner = (
            ROOT
            / "scripts"
            / "run_massive_medical_kalai_s1_recovery_continuation_unattended_v2_tillicum.sh"
        ...
        self.assertIn("python_bin=$root/envs/subliminal-mitigate-py311/bin/python", runner)
        self.assertEqual(runner.count('"$python_bin" "$manager"'), 4)
        self.assertNotIn("module load conda", runner)
        self.assertNotIn("conda activate", runner)
        self.assertIn("unset CONDA_PREFIX CONDA_DEFAULT_ENV", runner)

    def test_failed_v1_is_bound_as_zero_attempt_consumed_history(self):
        self.assertEqual(
            module.EXPECTED_V1_REPOSITORY_COMMIT,
            "49bc6297bc401a5c9c865964f234774217f12719",
        )
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn('"exact_slurm_submission_attempts": 0', source)
        self.assertIn('"actual_h200_cost_usd": 0.0', source)
        self.assertIn('"authority_consumed_and_nonreusable": True', source)
        self.assertIn('"cost_exposure_retained_usd": 0.0', source)
        self.assertIn("failed unattended-v1 STOPPED record differs", source)

    def test_stage_requires_both_failed_v1_inputs(self):
        stage = (
            ROOT
            / "scripts"
            / "stage_massive_medical_kalai_s1_recovery_continuation_unattended_v2_tillicum.sh"
        ).read_text(encoding="utf-8")
        self.assertEqual(stage.count('--v1-output-root "$v1_output"'), 2)
        self.assertEqual(stage.count('--v1-repo-root "$v1_repo"'), 2)
        self.assertIn('test "$(find "$v1_output" -type f | wc -l)" = 6', stage)
        self.assertIn('test -s "$v1_output/control/SEQUENCE_STOPPED"', stage)
        self.assertIn('test ! -e "$v1_output/control/batches/batch_03/SACCT.tsv"', stage)
