# Kalai s=1 recovered-batch-2 continuation v2
...
did not own the audited function. The batch-2 recovery corrects that evaluator
binding without regeneration. Continuation v2 prevents recurrence by routing
...
3. the fresh batch-2 recovery plan, CPU stage, and recovered result;
4. recomputation of the recovered result from the same sealed source evidence;
5. absence of batch-3 state in the source and fresh v2 namespaces.

Staging writes only `RECOVERY_CONTINUATION_PLAN.json` and `CPU_STAGE.json` in
the fresh v2 namespace. Apart from transferring the versioned repository to
...
60 minutes, and a retained `$0.900` cap. The first v2 batch (original batch 3)
must bind the recovered batch-2 result; later batches must bind the immediately
preceding valid v2 result. There is no automatic next batch, restart, resume,
